...
IsiMessage isi_out;
IsiType gIsiType =
        isiTypeS;  // Default to ISI Type S (simple with no domain address server)
IsiFlags gIsiFlags;
unsigned char gIsiDerivableAddr =
        0;  // Flag to indicate whether the IP address is derivabled or not
IzotReadOnlyData read_only_data;
IzotConfigData config_data;
IzotDomain domainTable
        [MAX_DOMAINS];  // Retrieve using IzotQueryDomainConfig(const unsigned index, IzotDomain* const pDomain);
IzotAddress
        addrTable;  // Retrieve using IzotQueryAddressConfig(const unsigned index, IzotAddress* const pAddress);
