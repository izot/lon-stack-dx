        ...
        int nonCloneValue, IzotBool bUpdateID)
{
    ...
    if (domainIndex <= IZOT_GET_ATTRIBUTE(read_only_data, IZOT_READONLY_TWO_DOMAINS)) {
        ...
        IZOT_SET_ATTRIBUTE_P(temp, IZOT_DOMAIN_NONCLONE,
                nonCloneValue);  // 0 = if it's a clone domain, 1= otherwise
        IZOT_SET_ATTRIBUTE_P(temp, IZOT_DOMAIN_NODE,
                IZOT_GET_ATTRIBUTE_P(domainConfig, IZOT_DOMAIN_NODE));
        temp->InvalidIdLength = domainConfig->InvalidIdLength;
        IZOT_SET_ATTRIBUTE_P(temp, IZOT_DOMAIN_INVALID,
                0);  // set the domain to be valid.  Otherwise, the LTS will reset the length to 7
        sts = IzotUpdateDomainConfig(domainIndex, temp);
        _IsiAPIDebug("DomainID  = %x %x %x %x %x %x, Subnet=%d, NonClone=%d Node=%d "
                     "Invalid=%d Length=%d Key=%x\n",
                temp->Id[0], temp->Id[1], temp->Id[2], temp->Id[3], temp->Id[4],
                temp->Id[5], temp->Subnet,
...
unsigned int LonIsiAddressTableCount(void)
{
    ...
    return (addressTableCount <= ISI_MAX_ADDRESS_TABLE_SIZE) ? addressTableCount
                                                             : ISI_MAX_ADDRESS_TABLE_SIZE;
}
 ...
 */
IsiType LonIsiType() { return gIsiType; }
 ...
 */
void LonIsiSetType(IsiType type) { gIsiType = type; }
