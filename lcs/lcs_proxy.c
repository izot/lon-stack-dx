...
void processProxyRepeaterAsAgent(APPReceiveParam *appReceiveParamPtr, APDU *apduPtr,
        int txcpos)
{
    ...
    if (len < appReceiveParamPtr->pduSize &&
...
LonStatusCode ProcessLtepCompletion(APPReceiveParam *appReceiveParamPtr, APDU *apduPtr,
...
LonStatusCode ProcessLTEP(APPReceiveParam *appReceiveParamPtr, APDU *apduPtr)
{
    ...
    txcpos = (int)(sizeof(ProxyHeader) +
                   (uniform ? sizeof(ProxySubnetNodeAddressCompact) * count
    ...
    if (count != 0) {
        ...
        if (ph.long_timer || txc.timer >= 10) {
            adjust = 512 * count;  // Add 512 msec per hop
        } else if (txc.timer >= 8) {
            adjust = 256 * count;  // Add 256 msec per hop
        }
...

        {
            ...
            if (pProxySicb->mode == PROXY_AGENT_MODE_ALTKEY) {
                ...
                len = (unsigned char)(pKey->type == AUTH_OMA
                                              ? sizeof(((ProxyOmaKey *)pKey)->key)
                                              : sizeof(pKey->key));
                ...
                while (domainIndex < 2) {
                    ...
                    for (j = 0; j < IZOT_DOMAIN_ID_MAX_LENGTH; j++) {
                        altKey.altKeyValue[i][j] =
                                eep->domainTable[domainIndex].Key[j] + *pKeyDelta;
                        pKeyDelta++;
                    }
                    domainIndex++;
                    i++;
                ...
                pAddress = (ProxyTargetAddress *)((unsigned char *)pAddress +
                                                  (int)(sizeof(ProxyAuthKey) -
                                                          sizeof(pKey->key) + len));
            }

            switch (pProxySicb->type) {
            case PX_NEURON_ID:
                memcpy(addr.uniqueNodeId.NeuronId, pAddress->nid.nid,
                        sizeof(addr.uniqueNodeId.NeuronId));
                subnet = pAddress->nid.subnet;
                break;
            case PX_NEURON_ID_COMPACT:
                memcpy(addr.uniqueNodeId.NeuronId, pAddress->nidc.nid,
                        sizeof(addr.uniqueNodeId.NeuronId));
                subnet = 0;
                break;
    ...
    addr.snode.domainIndex =
            domIdx;  // Set the domain index to be the one in which it was received.
    if (addressType & 0x80) {
    ...
    if (appReceiveParamPtr->priority) {
        outQPtr =
                (service == IzotServiceUnacknowledged) ? &gp->nwOutPriQ : &gp->tsaOutPriQ;
    } else {
    ...
    if (QueueFull(outQPtr)) {
        ...
        if (LonTimerExpired(&gp->proxyBufferWait)) {
            SendResponse(appReceiveParamPtr->reqId, LT_ENHANCED_PROXY_FAILURE,
                    sizeof(proxyCount), &proxyCount);
    ...
    if (dataLen >= (int)(sizeof(ProxyHeader) + sizeof(ProxySicb) + 1) &&
            offset <= dataLen) {
        LonStatusCode sts = LonStatusNoError;
        dataLen -= offset;
        if (service == IzotServiceUnacknowledged) {
            PktCtrl ctrl = alt ? PKT_ALTPATH | PKT_PROXY : PKT_PROXY;
            sts = AllocSendUnackd(ctrl, appReceiveParamPtr->reqId, &addr, code,
                    pData[offset], dataLen, &pData[offset + 1]);
