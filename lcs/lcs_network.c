...
LonStatusCode NetworkLayerReset(void)
{
    ...
    if (!LON_SUCCESS(status = DecodeBufferSize(NW_IN_BUF_SIZE, &gp->nwInBufSize))) {
        ...
        OsalPrintLog(ERROR_LOG, status,
                "NetworkLayerReset: Unable to decode input network buffer size");
        return status;
    }
    if (!LON_SUCCESS(
                status = DecodeBufferCnt((IzotByte)IZOT_GET_ATTRIBUTE(eep->readOnlyData,
                                                 IZOT_READONLY_NW_INBUF_CNT),
                        &gp->nwInQCnt))) {
        gp->resetOk = FALSE;
        OsalPrintLog(ERROR_LOG, status,
                "NetworkLayerReset: Unable to decode input network queue count");
        return status;
    }
    queueItemSize = gp->nwInBufSize + sizeof(NWReceiveParam);

    if (!LON_SUCCESS(status = QueueInit(&gp->nwInQ, "network layer input", queueItemSize,
                             gp->nwInQCnt))) {
        gp->resetOk = FALSE;
        OsalPrintLog(ERROR_LOG, status,
                "NetworkLayerReset: Unable to initialize the input queue");
        return status;
    }

    /* Allocate and initialize the output queue. */
    if (!LON_SUCCESS(status = DecodeBufferSize(IZOT_GET_ATTRIBUTE(eep->readOnlyData,
                                                       IZOT_READONLY_NW_OUTBUF_SIZE),
                             &gp->nwOutBufSize))) {
        gp->resetOk = FALSE;
        OsalPrintLog(ERROR_LOG, status,
                "NetworkLayerReset: Unable to decode output network buffer size");
        return status;
    }
    if (!LON_SUCCESS(
                status = DecodeBufferCnt((IzotByte)IZOT_GET_ATTRIBUTE(eep->readOnlyData,
        ...
        gp->resetOk = FALSE;
        OsalPrintLog(ERROR_LOG, status,
                "NetworkLayerReset: Unable to decode output network queue count");
        return status;
    }

    if (!LON_SUCCESS(status = QueueInit(&gp->nwOutQ, "network layer output",
                             queueItemSize, gp->nwOutQCnt))) {
        gp->resetOk = FALSE;
        OsalPrintLog(ERROR_LOG, status,
                "NetworkLayerReset: Unable to initialize the output queue");
        return status;
    }
    ...
    if (!LON_SUCCESS(
        ...
        gp->resetOk = FALSE;
        OsalPrintLog(ERROR_LOG, status,
                "NetworkLayerReset: Unable to decode priority output network queue "
                "count");
        return status;
    }
    ...
    if (gp->nwOutPriQCnt < 1) {
        ...
        OsalPrintLog(ERROR_LOG, LonStatusInvalidBufferCount,
                "NetworkLayerReset: Network Layer Priority Output queue count must be at "
                "least 1");
        return status;
    }

    if (!LON_SUCCESS(status = QueueInit(&gp->nwOutPriQ, "network layer priority output",
                             queueItemSize, gp->nwOutPriQCnt))) {
        gp->resetOk = FALSE;
        OsalPrintLog(ERROR_LOG, status,
                "NetworkLayerReset: Unable to initialize the priority output queue");
        return status;
    }
...
void NWSendTerminate(void)
{
    ...
    if (nwSendParamPtr->pduType == APDU_TYPE) {
        ...
        appReceiveParamPtr->proxyCount =
                0;  // This technically isn't correct but this case should never happen on a working system.
...
void NetworkLayerSend(void)
{
    ...
    OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
            "NetworkLayerSend: Sending a network layer packet to the link layer");
    nwSendParamPtr = QueuePeek(gp->nwCurrent);
    ...
    if (nwSendParamPtr->pduType == APDU_TYPE && QueueFull(&gp->appCeRspInQ)) {
        ...
        OsalPrintLog(ERROR_LOG, LonStatusNoBufferAvailable,
                "NetworkLayerSend: No room for completion event notification");
        return;
    }
    ...
    default:
        ...
        OsalPrintLog(ERROR_LOG, LonStatusInvalidMessageMode,
                "NetworkLayerSend: Unknown address mode (0x%02X)",
                nwSendParamPtr->destAddr.addressMode);
        NWSendTerminate();
        return;
    }
    ...
    if (nwSendParamPtr->destAddr.dmn.domainIndex < numDomains &&
            IZOT_GET_ATTRIBUTE(eep->domainTable[nwSendParamPtr->destAddr.dmn.domainIndex],
                    IZOT_DOMAIN_INVALID)) {
        if (!nwSendParamPtr->dropIfUnconfigured) {
            ...
            OsalPrintLog(INFO_LOG, LonStatusInvalidDomain,
                    "NetworkLayerSend: Discarding packet due to domain index %d is not "
                    "in use",
                    nwSendParamPtr->destAddr.dmn.domainIndex);
        }
        OsalPrintLog(ERROR_LOG, LonStatusInvalidDomain,
                "NetworkLayerSend: Domain index %d is not in use",
                nwSendParamPtr->destAddr.dmn.domainIndex);
        NWSendTerminate();
        return;
    }
    ...
    if (nwSendParamPtr->destAddr.dmn.domainIndex < numDomains) {
        ...
        nwSendParamPtr->destAddr.dmn.domainLen = IZOT_GET_ATTRIBUTE(
                eep->domainTable[nwSendParamPtr->destAddr.dmn.domainIndex],
                IZOT_DOMAIN_ID_LENGTH);
        memcpy(nwSendParamPtr->destAddr.dmn.domainId,
                eep->domainTable[nwSendParamPtr->destAddr.dmn.domainIndex].Id,
                nwSendParamPtr->destAddr.dmn.domainLen);

        npduPtr->data[0] =
                eep->domainTable[nwSendParamPtr->destAddr.dmn.domainIndex].Subnet;
        npduPtr->data[1] =
                selField << 7 |
                IZOT_GET_ATTRIBUTE(
                        eep->domainTable[nwSendParamPtr->destAddr.dmn.domainIndex],
    ...
    if (EncodeDomainLength(nwSendParamPtr->destAddr.dmn.domainLen, &domainLength) !=
            LonStatusNoError) {
        // Domain length is invalid
        OsalPrintLog(ERROR_LOG, LonStatusInvalidDomain,
                "NetworkLayerSend: Invalid domain length (%d)",
                nwSendParamPtr->destAddr.dmn.domainLen);
        NWSendTerminate();
        return;
    }
    ...
    case AM_MULTICAST:
        ...
        break;
    case AM_SUBNET_NODE:
        IZOT_SET_ATTRIBUTE(nwSendParamPtr->destAddr.addr.addr2a, IZOT_RECEIVESN_SELFIELD,
                1);
        memcpy(&npduPtr->data[2], &nwSendParamPtr->destAddr.addr.addr2a, 2);
        j = 4;
        break;
    case AM_MULTICAST_ACK:
        IZOT_SET_ATTRIBUTE(nwSendParamPtr->destAddr.addr.addr2b.subnetAddr,
    ...
    OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
            "NetworkLayerSend: Sending a %d byte packet to the link layer", npduSize);
...
void NetworkLayerReceive(void)
{
    ...
    default:
        ...
        OsalPrintLog(ERROR_LOG, LonStatusBadAddressType,
                "NetworkLayerReceive: Unknown address format");
    ...
    if (domainLength != 0 && domainLength != 1 && domainLength != 3) {
        OsalPrintLog(ERROR_LOG, LonStatusInvalidDomain,
                "NetworkLayerReceive: Invalid domain length (%d)", domainLength);
    ...
    if (NodeConfigured() &&
            ...
            domainLength ==
                    IZOT_GET_ATTRIBUTE(eep->domainTable[0], IZOT_DOMAIN_ID_LENGTH) &&
        ...
        srcAddr.dmn.domainIndex = 0;
               ...
               domainLength ==
                       IZOT_GET_ATTRIBUTE(eep->domainTable[1], IZOT_DOMAIN_ID_LENGTH) &&
    ...
    if (!flexDomain &&
            memcmp(&srcAddr.subnetAddr, &eep->domainTable[srcAddr.dmn.domainIndex].Subnet,
                    2) == 0 &&
            srcAddr.dmn.domainIndex != 1) {
        /* Not flex domain and source address matches */
        QueueDropHead(&gp->nwInQ); /* Discard packet */
        OsalPrintLog(INFO_LOG, LonStatusNoError,
                "NetworkLayerReceive: Discard message from me with subnet %x - %x",
    ...
    case AM_BROADCAST:
        if (!flexDomain && destAddr.Subnet != 0 &&
                memcmp(&destAddr.Subnet,
                        &eep->domainTable[srcAddr.dmn.domainIndex].Subnet, 1) != 0) {
            /* Subnet broadcast and domain matches but destAddr does not. Not for us. */
            QueueDropHead(&gp->nwInQ);
            OsalPrintLog(INFO_LOG, LonStatusNoError,
                    "NetworkLayerReceive: Discard broadcast packet not in my subnet");
            return;
        }
    ...
    case AM_MULTICAST:
        if (!flexDomain &&
                !IsGroupMember(srcAddr.dmn.domainIndex, srcAddr.group.GroupId, NULL)) {
            /* Domain matches but group does not. Not for us. */
            QueueDropHead(&gp->nwInQ);
            OsalPrintLog(INFO_LOG, LonStatusNoError,
                    "NetworkLayerReceive: Discard multicast packet not in my group");
            return;
        }
        break;
    case AM_SUBNET_NODE:
        if (!flexDomain &&
                memcmp(&destAddr, &eep->domainTable[srcAddr.dmn.domainIndex].Subnet, 2) !=
                        0) {
            QueueDropHead(&gp->nwInQ);
            OsalPrintLog(INFO_LOG, LonStatusNoError,
                    "NetworkLayerReceive: Discard multicast packet not in my subnet");
            return;
        }
        break;
    case AM_MULTICAST_ACK:
        ...
        if (!flexDomain &&
            ...
            QueueDropHead(&gp->nwInQ);
            OsalPrintLog(INFO_LOG, LonStatusNoError,
                    "NetworkLayerReceive: Discard multicast ack packet not to me");
            return;
        }
        /* Also make sure that group matches. */
        if (!flexDomain && !IsGroupMember(srcAddr.dmn.domainIndex,
                                   srcAddr.ackNode.groupAddr.group.GroupId, NULL)) {
            QueueDropHead(&gp->nwInQ);
            OsalPrintLog(INFO_LOG, LonStatusNoError,
                    "NetworkLayerReceive: Discard multicast ack packet not my group");
            return;
        }
        break;
    case AM_UNIQUE_NODE_ID:
        if (memcmp(uniqueNodeId, eep->readOnlyData.UniqueNodeId, IZOT_UNIQUE_ID_LENGTH) !=
                0) {
            /* Unique Node Id message but not for our id. */
            QueueDropHead(&gp->nwInQ);
            OsalPrintLog(INFO_LOG, LonStatusNoError,
                    "NetworkLayerReceive: Discard Unique ID packet that is not my ID");
            return;
        }
        break;
    default:
        ...
        QueueDropHead(&gp->nwInQ);
        OsalPrintLog(ERROR_LOG, LonStatusInvalidMessageMode,
                "NetworkLayerReceive: Invalid message mode");
        return;
    }

    /* If a node is in unconfigured state,
       only broadcast and Unique ID messages can be received. */
    if (!NodeConfigured() && srcAddr.addressMode != AM_BROADCAST &&
            srcAddr.addressMode != AM_UNIQUE_NODE_ID) {
        /* Drop the packet. */
        QueueDropHead(&gp->nwInQ);
        OsalPrintLog(INFO_LOG, LonStatusNoError,
                "NetworkLayerReceive: Discard packet received while not configured");
        return;
    }
    ...
    if (flexDomain && NodeConfigured() && srcAddr.addressMode != AM_UNIQUE_NODE_ID) {
        ...
        QueueDropHead(&gp->nwInQ);
        OsalPrintLog(INFO_LOG, LonStatusNoError,
                "NetworkLayerReceive: Discard flex domain packet that is not UID "
                "addressed");
        return;
    }
    ...
    if (nwReceiveParamPtr->pduSize <= j + 1) {
        ...
        QueueDropHead(&gp->nwInQ);
        OsalPrintLog(INFO_LOG, LonStatusNoError,
                "NetworkLayerReceive: Discard short packet");
        return;
    }
    ...
    case APDU_TYPE:
        if (QueueFull(&gp->appInQ) || pduSize > gp->appInBufSize) {
            ...
            if (pduSize > gp->appInBufSize) {
                OsalPrintLog(ERROR_LOG, LonStatusWritePastEndOfApplBuffer,
                        "NetworkLayerReceive: Packet size (%d) exceeds application "
                        "buffer size (%d)",
            ...
            QueueDropHead(&gp->nwInQ);
            OsalPrintLog(ERROR_LOG, LonStatusNoBufferAvailable,
                    "NetworkLayerReceive: No space in application layer input queue");
            return;
        }
        ...
        if (npduPtr->data[j] == LT_AES_GCM_PACKET_CODE) {
            if (processAesGcm(true, appReceiveParamPtr->service,
                        appReceiveParamPtr->priority, &npduPtr->data[j],
                        appReceiveParamPtr
                                ->pduSize, /* Length of original Encrypted APDU */
                        (uint8_t *)npduPtr, j)) {
                return;
            }
        }
    ...
    case AUTHPDU_TYPE:
        if (QueueFull(&gp->tsaInQ) || pduSize > gp->tsaInBufSize) {
            ...
            if (pduSize > gp->tsaInBufSize) {
                ...
                OsalPrintLog(ERROR_LOG, LonStatusWritePastEndOfApplBuffer,
                        "NetworkLayerReceive: Packet size (%d) exceeds TSA buffer size "
                        "(%d)",
            ...
            QueueDropHead(&gp->nwInQ);
            OsalPrintLog(ERROR_LOG, LonStatusNoBufferAvailable,
                    "NetworkLayerReceive: No space in authentication input queue");
            return;
        }
        /* Queue is not full and buffer has sufficient space. */
    ...
    default:
        OsalPrintLog(ERROR_LOG, LonStatusUnknownPdu,
                "NetworkLayerReceive: Unknown PDU type (0x%02X) received",
                npduPtr->pduType);
        QueueDropHead(&gp->nwInQ);
        return;
    }
