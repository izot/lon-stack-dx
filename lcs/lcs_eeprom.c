...
LonStatusCode LCS_ReadPersistentNetworkImage(void)
{
    ...
    if (status == LonStatusNoError) {
        OsalPrintLog(INFO_LOG, LonStatusNoError,
                "LCS_ReadPersistentNetworkImage: Restored network image from "
                "non-volatile memory");
    } else if (status == LonStatusPersistentDataFailure) {
        OsalPrintLog(INFO_LOG, LonStatusNoError,
                "LCS_ReadPersistentNetworkImage: Failed to restore network image from "
                "NVM--may be first boot");
    } else {
...
LonStatusCode LCS_ReadPersistentAppData(void)
{
    ...
    if (status == LonStatusNoError) {
        OsalPrintLog(INFO_LOG, LonStatusNoError,
                "LCS_ReadPersistentAppData: Restored application data from non-volatile "
                "memory");
    } else {
        OsalPrintLog(INFO_LOG, status,
                "LCS_ReadPersistentAppData: Failed to restore application data from "
                "non-volatile memory--may be first boot");
    }
    return (status);
}
