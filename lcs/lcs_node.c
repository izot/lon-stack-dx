...
static const Dimensions dimensions = {MAX_DOMAINS, NUM_ADDR_TBL_ENTRIES, NV_TABLE_SIZE,
        NV_ALIAS_TABLE_SIZE};
...
static const IzotUbits16 bufSizeCodeLGbl[16] = {255, 20, 20, 21, 22, 24, 26, 30, 34, 42,
        50, 66, 82, 114, 146, 210};
static const IzotUbits16 bufCntCodeLGbl[16] = {0, 1, 1, 2, 3, 5, 7, 11, 15, 23, 31, 47,
        63, 95, 127, 191};
static const IzotUbits16 rptTimerCodeLGbl[16] = {16, 24, 32, 48, 64, 96, 128, 192, 256,
        384, 512, 768, 1024, 1536, 2048, 3072};
static const IzotUbits16 rcvTimerCodeLGbl[16] = {128, 192, 256, 384, 512, 768, 1024, 1536,
...
IzotDomain *AccessDomain(IzotByte indexIn)
{
    if (indexIn <= IZOT_GET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_TWO_DOMAINS)) {
        ...
        OsalPrintLog(INFO_LOG, LonStatusNoError,
                "AccessDomain: Domain %x%x%x%x%x%x, Subnet %d, NonClone %d, Node %d, "
                "Invalid %d, Length %d, Key %x",
                domain_config->Id[0], domain_config->Id[1], domain_config->Id[2],
                domain_config->Id[3], domain_config->Id[4], domain_config->Id[5],
                domain_config->Subnet,
                ...
                IZOT_GET_ATTRIBUTE_P(domain_config, IZOT_DOMAIN_ID_LENGTH),
                domain_config->Key[0]);
        ...
        IzotByte includeKey)
{
    ...
    int nDomains = IZOT_GET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_TWO_DOMAINS)
                           ? MAX_DOMAINS
                           : 1;
    if (indexIn < nDomains) {
        memcpy(&eep->domainTable[indexIn], domainInp,
                includeKey ? sizeof(IzotDomain)
...
LonStatusCode UpdateAddress(const IzotAddress *addrEntryInp, IzotUbits16 indexIn)
{
    ...
    if (indexIn < eep->readOnlyData.Extended) {
        ...
        OsalPrintLog(ERROR_LOG, LonStatusInvalidAddrTableIndex,
                "UpdateAddress: Invalid address table index");
...
******************************************************************/
IzotByte IsGroupMember(IzotByte domainIndexIn, IzotByte groupIn, IzotByte *groupMemberOut)
{
    IzotUbits16 i;
    ...
    for (i = 0; i < NUM_ADDR_TBL_ENTRIES; i++) {
        if (IZOT_GET_ATTRIBUTE(eep->addrTable[i].Group, IZOT_ADDRESS_GROUP_TYPE) == 1) {
            ...
            /* Group Format */
            if (eep->addrTable[i].Group.Group == groupIn &&
                    IZOT_GET_ATTRIBUTE(eep->addrTable[i].Group,
    ...
    if (groupMemberOut) {
        *groupMemberOut =
                IZOT_GET_ATTRIBUTE(eep->addrTable[i].Group, IZOT_ADDRESS_GROUP_MEMBER);
    }
    return (TRUE); /* Found */
}
...
LonStatusCode DecodeBufferSize(IzotByte bufSizeIn, uint16_t *decodedSizeOut)
{
    ...
    OsalPrintLog(ERROR_LOG, LonStatusInvalidBufferSize,
            "DecodeBufferSize: Invalid buffer size code %d", bufSizeIn);
...
LonStatusCode DecodeBufferCnt(IzotByte bufCntIn, uint16_t *decodedCountOut)
{
    ...
    OsalPrintLog(ERROR_LOG, LonStatusInvalidBufferCount,
            "DecodeBufferCnt: Invalid buffer count code");
...
void UpdateNV(IzotDatapointConfig *nvStructInp, IzotUbits16 indexIn)
{
    ...
    if (nvStructInp) {
        OsalPrintLog(ERROR_LOG, LonStatusInvalidDatapointIndex,
                "UpdateNV: Invalid index");
    } else {
        OsalPrintLog(ERROR_LOG, LonStatusInvalidDatapointIndex,
                "UpdateNV: Invalid NV configuration table update");
    }
}
...
void UpdateAlias(IzotAliasConfig *aliasStructInp, IzotUbits16 indexIn)
{
    ...
    if (aliasStructInp) {
        OsalPrintLog(ERROR_LOG, LonStatusInvalidDatapointIndex,
                "UpdateAlias: Invalid index");
    } else {
        OsalPrintLog(ERROR_LOG, LonStatusInvalidDatapointIndex,
                "UpdateAlias: Invalid alias configuration table update");
    }
}
...
LonStatusCode NodeReset(IzotByte firstReset)
{
    ...
    OsalPrintLog(INFO_LOG, status,
            "NodeReset: Start LON application reset with firstReset=%d", firstReset);
#if LINK_IS(UDP)
    ...
    LonStatusCode (*resetFns[])(void) = {AppLayerReset, TransactionControlSublayerReset,
            TransactionServicesSublayerReset, NetworkLayerReset, LinkLayerUdpReset};
#elif LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
    LonStatusCode AppLayerReset(void), TransactionControlSublayerReset(void),
            TransactionServicesSublayerReset(void), NetworkLayerReset(void),
            LinkLayerReset(void);
    LonStatusCode (*resetFns[])(void) = {AppLayerReset, TransactionControlSublayerReset,
            TransactionServicesSublayerReset, NetworkLayerReset, LinkLayerReset};
#elif LINK_IS(SPI_MIP)
    LonStatusCode AppLayerReset(void), TransactionControlSublayerReset(void),
            TransactionServicesSublayerReset(void), NetworkLayerReset(void),
    ...
    OsalPrintLog(ERROR_LOG, LonStatusInitializationFailed,
            "NodeReset: Unsupported link type");
    return;
#endif  // LINK_IS(UDP)
    ...
    if (!gp->initialized) {
        ...
        if (status == LonStatusInvalidParameter) {
            ...
            OsalPrintLog(ERROR_LOG, status,
                    "InitEEPROM: Non-volatile memory image too large");
            ...
            IZOT_SET_ATTRIBUTE(eep->configData, IZOT_CONFIG_COMM_TYPE, SPECIAL_PURPOSE);
            IZOT_SET_ATTRIBUTE(eep->configData, IZOT_CONFIG_COMM_PINDIR,
                    0x1E);                              /* 0x17 if wake-up pin is input */
            ...
            for (i = 0; i <= cp->twoDomains; i++) {
                IZOT_SET_ATTRIBUTE(eep->domainTable[i], IZOT_DOMAIN_ID_LENGTH,
                        cp->len[i]);
                ...
                IZOT_SET_ATTRIBUTE(eep->domainTable[i], IZOT_DOMAIN_NONCLONE,
                        cp->clone[i]);
                memcpy(eep->domainTable[i].Key, cp->key[i],
        ...
        IZOT_SET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_MINORNUM, MINOR_ARCH_NUM);
        ...
        IZOT_SET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_DATAPOINT_PROCESSINGOFF, 0);
        ...
        IZOT_SET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_OUTBUF_SIZE,
                APP_OUT_BUF_SIZE);
        IZOT_SET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_INBUF_SIZE, APP_IN_BUF_SIZE);
        IZOT_SET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_NW_OUTBUF_SIZE,
                NW_OUT_BUF_SIZE);
        IZOT_SET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_NW_INBUF_SIZE,
                NW_IN_BUF_SIZE);
        IZOT_SET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_NW_OUT_PRICNT,
                NW_OUT_PRI_Q_CNT);
        IZOT_SET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_OUT_PRICNT,
                APP_OUT_PRI_Q_CNT);
        ...
        IZOT_SET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_MSG_TAG_CNT,
                BindableMTagCount);
        IZOT_SET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_RESERVED10, 0x8);

        IZOT_SET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_RW_PROTECT,
                READ_WRITE_PROTECT);
        IZOT_SET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_TX_BY_ADDRESS, 0);
        IZOT_SET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_ALIAS_CNT,
                0);                                     /* Host based node */
...
// Record the dimensions uses for NVM.  If these change, we'll reset all the NVM
        ...
        snvt_capability_info->address_capacity =
                (cp->addressCnt <= 15) ? hton16(cp->addressCnt) : hton16(15);
...
IzotByte IsNVBound(IzotBits16 nvIndexIn)
{
    ...
    addrIndex = ADDR_INDEX(IZOT_GET_ATTRIBUTE(eep->nvConfigTable[nvIndexIn],
                                   IZOT_DATAPOINT_ADDRESS_HIGH),
            IZOT_GET_ATTRIBUTE(eep->nvConfigTable[nvIndexIn],
                    IZOT_DATAPOINT_ADDRESS_LOW));
    ...
    for (i = 0; i < NV_ALIAS_TABLE_SIZE; i++) {
        ...
        addrIndex = ADDR_INDEX(IZOT_GET_ATTRIBUTE(eep->nvAliasTable[i].Alias,
                                       IZOT_DATAPOINT_ADDRESS_HIGH),
                IZOT_GET_ATTRIBUTE(eep->nvAliasTable[i].Alias,
                        IZOT_DATAPOINT_ADDRESS_LOW));
...
IzotBool AppPgmRuns(void)
{
    ...
    if (IZOT_GET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_NODE_STATE) ==
                    IzotApplicationUnconfig &&
            IZOT_GET_ATTRIBUTE(eep->readOnlyData, IZOT_READONLY_RUN_UNCONFIG) &&
            gp->appPgmMode == ON_LINE) {
        return (TRUE);
    }
...
void DoApp(IzotBool isOnline)
{
    ...
    if (isOnline) {
        if (MsgReceive(&msg_in)) {
            if (!IzotFilterMsgArrived(&msg_in->addr, (IzotCorrelator)&msg_in->reqId, 0,
                        msg_in->service, msg_in->authenticated, msg_in->code,
                        msg_in->data, msg_in->len)) {
                IzotMsgArrived(&msg_in->addr, (IzotCorrelator)&msg_in->reqId, 0,
                        msg_in->service, msg_in->authenticated, msg_in->code,
                        msg_in->data, msg_in->len);
            }
            MsgFree();
        ...
        if (RespReceive(&rsp_in)) {
            if (!IzotFilterResponseArrived(&rsp_in->addr, rsp_in->tag, rsp_in->code,
                        rsp_in->data, rsp_in->len)) {
                IzotResponseArrived(&rsp_in->addr, rsp_in->tag, rsp_in->code,
                        rsp_in->data, rsp_in->len);
            }
            RespFree();
        }
 ...
 *
 ****************************************************************************/
uint8_t izot_get_device_mode(void) { return gp->appPgmMode; }
 ...
 *
 ****************************************************************************/
void izot_send_service_pin_wrapper(void) { ManualServiceRequestMessage(); }
...
{
    char version[10];
    snprintf(version, 10, "%d.%d.%02d", FIRMWARE_VERSION, FIRMWARE_MINOR_VERSION,
            FIRMWARE_BUILD);
    strcpy(cpm_sdk_version, version);
}

IzotBool IsPhysicalResetRequested(void) { return do_reset; }

void PhysicalResetRequested(void) { do_reset = TRUE; }
