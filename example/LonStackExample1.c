...
IzotDatapointDefinition
        heartbeatInDef;  // Used to create definition and get NV index.  Copied into NV config table.
...
IzotDatapointDefinition
        flow1InDef;  // Used to create definition and get NV index.  Copied into NV config table.
...
IzotDatapointDefinition
        flow2InDef;  // Used to create definition and get NV index.  Copied into NV config table.
...
IzotDatapointDefinition
        temp1InDef;  // Used to create definition and get NV index.  Copied into NV config table.
...
IzotDatapointDefinition
        temp2InDef;  // Used to create definition and get NV index.  Copied into NV config table.
...
IzotDatapointDefinition
        flow1OutDef;  // Used to create definition and get NV index.  Copied into NV config table.
...
IzotDatapointDefinition
        flow2OutDef;  // Used to create definition and get NV index.  Copied into NV config table.
...
IzotDatapointDefinition
        temp1OutDef;  // Used to create definition and get NV index.  Copied into NV config table.
...
IzotDatapointDefinition
        temp2OutDef;  // Used to create definition and get NV index.  Copied into NV config table.
...
void Example1DatapointUpdateOccurred(const unsigned index,
        const IzotReceiveAddress *const pSourceAddress);
void SetHeartbeatTimer(void);
void HeartbeatInUpdateOccurred(const unsigned index,
        const IzotReceiveAddress *const pSourceAddress);
void Flow1InUpdateOccurred(const unsigned index,
        const IzotReceiveAddress *const pSourceAddress);
void Flow2InUpdateOccurred(const unsigned index,
        const IzotReceiveAddress *const pSourceAddress);
void Temp1InUpdateOccurred(const unsigned index,
        const IzotReceiveAddress *const pSourceAddress);
void Temp2InUpdateOccurred(const unsigned index,
        const IzotReceiveAddress *const pSourceAddress);
...
LonStatusCode SetUpExample1(void)
{
    ...
    success = LON_SUCCESS(status = IzotCreateStack(&LonStackInterface,
              ...
              LON_SUCCESS(status = IzotUpdateDomain(0, EXAMPLE_DOMAIN_LENGTH,
                                  (IzotByte *)&domainId, EXAMPLE_SUBNET, EXAMPLE_NODE)) &&
              LON_SUCCESS(status = SetUpAddressTable()) &&
              LON_SUCCESS(status = IzotDatapointUpdateOccurredRegistrar(
                                  &Example1DatapointUpdateOccurred));
    ...
    if (LonTimerExpired(&HeartbeatTimer)) {
        if (service_count > 0) {
            OsalPrintLog(INFO_LOG, status,
                    "LoopExample1: Send Service message, attempt number %u",
                    INITIAL_SERVICE_COUNT - service_count + 1);
            if (!LON_SUCCESS(IzotSendServiceMessage())) {
                OsalPrintLog(ERROR_LOG, status,
                        "LoopExample1: Failed to send initial service message");
                return status;
            }
            service_count--;
            ...
            if (!LON_SUCCESS(IzotPropagateByIndex(flow1OutDef.NvIndex))) {
                OsalPrintLog(ERROR_LOG, status,
                        "LoopExample1: Failed to propagate flow1Out");
            }
            // TBD -- increment flow2Out
            if (!LON_SUCCESS(IzotPropagateByIndex(flow2OutDef.NvIndex))) {
                OsalPrintLog(ERROR_LOG, status,
                        "LoopExample1: Failed to propagate flow2Out");
            }
            ...
            if (!LON_SUCCESS(IzotPropagateByIndex(temp1OutDef.NvIndex))) {
                OsalPrintLog(ERROR_LOG, status,
                        "LoopExample1: Failed to propagate temp1Out");
            }
            // TBD -- increment temp2Out
            if (!LON_SUCCESS(IzotPropagateByIndex(temp2OutDef.NvIndex))) {
                OsalPrintLog(ERROR_LOG, status,
                        "LoopExample1: Failed to propagate temp2Out");
            }
        }
    }
...
LonStatusCode SetUpAddressTable(void)
{
    ...
    IZOT_SET_ATTRIBUTE(AddressTableEntry.SubnetNode, IZOT_ADDRESS_SN_NODE,
            EXAMPLE_TARGET_NODE);
...
LonStatusCode SetUpStaticNVs(void)
{
    ...
    // Specify flag configuration with: IzotDatapointFlags(IzotDatapointConfig DatapointConfig, IzotBool priority, IzotDatapointDirection direction,
    ...
    success = LON_SUCCESS(
                      status = IzotDatapointSetup(&heartbeatInDef, &heartbeatIn,
                              sizeof(heartbeatIn), SNVT_flow_p_index, 0, heartbeatInName,
                              heartbeatInSD, IZOT_DATAPOINT_RATE_UNKNOWN,
                              IZOT_DATAPOINT_RATE_UNKNOWN, NULL)) &&
              LON_SUCCESS(status = IzotDatapointFlags(&heartbeatInDef, FALSE,
                                  IzotDatapointDirectionIsInput, FALSE, FALSE, FALSE,
                                  FALSE)) &&
              LON_SUCCESS(status = IzotRegisterStaticDatapoint(&heartbeatInDef)) &&
              LON_SUCCESS(status = IzotDatapointBind(heartbeatInDef.NvIndex,
                                  HEARTBEAT_IN_ADDRESS, HEARTBEAT_IN_SELECTOR, FALSE,
    ...
    success = LON_SUCCESS(status = IzotDatapointSetup(&flow1InDef, &flow1In,
                                  sizeof(flow1In), SNVT_flow_p_index, 0, flow1InName,
                                  flow1InSD, IZOT_DATAPOINT_RATE_UNKNOWN,
                                  IZOT_DATAPOINT_RATE_UNKNOWN, NULL)) &&
              LON_SUCCESS(status = IzotDatapointFlags(&flow1InDef, FALSE,
                                  IzotDatapointDirectionIsInput, FALSE, FALSE, FALSE,
                                  FALSE)) &&
              LON_SUCCESS(status = IzotRegisterStaticDatapoint(&flow1InDef)) &&
              LON_SUCCESS(status = IzotDatapointBind(flow1InDef.NvIndex, FLOW1_IN_ADDRESS,
                                  FLOW1_IN_SELECTOR, FALSE, IzotServiceAcknowledged));
    if (!success) {
        return (status);
    }

    // SNVT_flow_f flow2In NV
    success = LON_SUCCESS(status = IzotDatapointSetup(&flow2InDef, &flow2In,
                                  sizeof(flow2In), SNVT_flow_f_index, 0, flow2InName,
                                  flow2InSD, IZOT_DATAPOINT_RATE_UNKNOWN,
                                  IZOT_DATAPOINT_RATE_UNKNOWN, NULL)) &&
              LON_SUCCESS(status = IzotDatapointFlags(&flow2InDef, FALSE,
                                  IzotDatapointDirectionIsInput, FALSE, FALSE, FALSE,
                                  FALSE)) &&
              LON_SUCCESS(status = IzotRegisterStaticDatapoint(&flow2InDef)) &&
              LON_SUCCESS(status = IzotDatapointBind(flow2InDef.NvIndex, FLOW2_IN_ADDRESS,
                                  FLOW2_IN_SELECTOR, FALSE, IzotServiceAcknowledged));
    if (!success) {
        return (status);
    }

    // SNVT_temp_p temp1In NV
    success = LON_SUCCESS(status = IzotDatapointSetup(&temp1InDef, &temp1In,
                                  sizeof(temp1In), SNVT_flow_p_index, 0, temp1InName,
                                  temp1InSD, IZOT_DATAPOINT_RATE_UNKNOWN,
                                  IZOT_DATAPOINT_RATE_UNKNOWN, NULL)) &&
              LON_SUCCESS(status = IzotDatapointFlags(&temp1InDef, FALSE,
                                  IzotDatapointDirectionIsInput, FALSE, FALSE, FALSE,
                                  FALSE)) &&
              LON_SUCCESS(status = IzotRegisterStaticDatapoint(&temp1InDef)) &&
              LON_SUCCESS(status = IzotDatapointBind(temp1InDef.NvIndex, TEMP1_IN_ADDRESS,
                                  TEMP1_IN_SELECTOR, FALSE, IzotServiceAcknowledged));
    if (!success) {
        return (status);
    }

    // SNVT_flow_f temp2In NV
    success = LON_SUCCESS(status = IzotDatapointSetup(&temp2InDef, &temp2In,
                                  sizeof(temp2In), SNVT_flow_f_index, 0, temp2InName,
                                  temp2InSD, IZOT_DATAPOINT_RATE_UNKNOWN,
                                  IZOT_DATAPOINT_RATE_UNKNOWN, NULL)) &&
              LON_SUCCESS(status = IzotDatapointFlags(&temp2InDef, FALSE,
                                  IzotDatapointDirectionIsInput, FALSE, FALSE, FALSE,
                                  FALSE)) &&
              LON_SUCCESS(status = IzotRegisterStaticDatapoint(&temp2InDef)) &&
              LON_SUCCESS(status = IzotDatapointBind(temp2InDef.NvIndex, TEMP2_IN_ADDRESS,
                                  TEMP2_IN_SELECTOR, FALSE, IzotServiceAcknowledged));
    if (!success) {
        return (status);
    }
    ...
    success =
            LON_SUCCESS(status = IzotDatapointSetup(&flow1OutDef, &flow1Out,
                                sizeof(flow1Out), SNVT_flow_p_index, 0, flow1OutName,
                                flow1OutSD, IZOT_DATAPOINT_RATE_UNKNOWN,
                                IZOT_DATAPOINT_RATE_UNKNOWN, NULL)) &&
            LON_SUCCESS(status = IzotDatapointFlags(&flow1OutDef, FALSE,
                                IzotDatapointDirectionIsOutput, FALSE, FALSE, FALSE,
                                FALSE)) &&
            LON_SUCCESS(status = IzotRegisterStaticDatapoint(&flow1OutDef)) &&
            LON_SUCCESS(status = IzotDatapointBind(flow1OutDef.NvIndex, FLOW1_OUT_ADDRESS,
                                FLOW1_OUT_SELECTOR, FALSE, IzotServiceAcknowledged));
    if (!success) {
        return (status);
    }
    ...
    success =
            LON_SUCCESS(status = IzotDatapointSetup(&flow2OutDef, &flow2Out,
                                sizeof(flow2Out), SNVT_flow_f_index, 0, flow2OutName,
                                flow2OutSD, IZOT_DATAPOINT_RATE_UNKNOWN,
                                IZOT_DATAPOINT_RATE_UNKNOWN, NULL)) &&
            LON_SUCCESS(status = IzotDatapointFlags(&flow2OutDef, FALSE,
                                IzotDatapointDirectionIsOutput, FALSE, FALSE, FALSE,
                                FALSE)) &&
            LON_SUCCESS(status = IzotRegisterStaticDatapoint(&flow2OutDef)) &&
            LON_SUCCESS(status = IzotDatapointBind(flow2OutDef.NvIndex, FLOW2_OUT_ADDRESS,
                                FLOW2_OUT_SELECTOR, FALSE, IzotServiceAcknowledged));
    if (!success) {
        return (status);
    }
    ...
    success =
            LON_SUCCESS(status = IzotDatapointSetup(&temp1OutDef, &temp1Out,
                                sizeof(temp1Out), SNVT_flow_p_index, 0, temp1OutName,
                                temp1OutSD, IZOT_DATAPOINT_RATE_UNKNOWN,
                                IZOT_DATAPOINT_RATE_UNKNOWN, NULL)) &&
            LON_SUCCESS(status = IzotDatapointFlags(&temp1OutDef, FALSE,
                                IzotDatapointDirectionIsOutput, FALSE, FALSE, FALSE,
                                FALSE)) &&
            LON_SUCCESS(status = IzotRegisterStaticDatapoint(&temp1OutDef)) &&
            LON_SUCCESS(status = IzotDatapointBind(temp1OutDef.NvIndex, TEMP1_OUT_ADDRESS,
                                TEMP1_OUT_SELECTOR, FALSE, IzotServiceAcknowledged));
    if (!success) {
        return (status);
    }
    ...
    success =
            LON_SUCCESS(status = IzotDatapointSetup(&temp2OutDef, &temp2Out,
                                sizeof(temp2Out), SNVT_flow_f_index, 0, temp2OutName,
                                temp2OutSD, IZOT_DATAPOINT_RATE_UNKNOWN,
                                IZOT_DATAPOINT_RATE_UNKNOWN, NULL)) &&
            LON_SUCCESS(status = IzotDatapointFlags(&temp2OutDef, FALSE,
                                IzotDatapointDirectionIsOutput, FALSE, FALSE, FALSE,
                                FALSE)) &&
            LON_SUCCESS(status = IzotRegisterStaticDatapoint(&temp2OutDef)) &&
            LON_SUCCESS(status = IzotDatapointBind(temp2OutDef.NvIndex, TEMP2_OUT_ADDRESS,
                                TEMP2_OUT_SELECTOR, FALSE, IzotServiceAcknowledged));
    if (!success) {
        return (status);
    }
...
void SetHeartbeatTimer(void)
{
    uint32_t heartbeatInterval = ((uint32_t)heartbeatIn.second * 1000) +
                                 (IZOT_GET_UNSIGNED_WORD(heartbeatIn.millisecond));
