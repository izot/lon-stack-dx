...
// Maximum time to wait for an uplink reset on startup and a layer mode change
// (milliseconds)
...
// Resynchronization delay to synchronize uplink and downlink state machines
// after a reset (milliseconds)
#ifndef NI_RESYNC_DELAY
#define NI_RESYNC_DELAY 300
#endif

// Maximum time to wait for a LON interface unique ID (UID) response
// (milliseconds)
...
// Maximum time to wait for an uplink acknowledgment during startup
// (milliseconds)
#ifndef STARTUP_ACK_WAIT_TIME
#define STARTUP_ACK_WAIT_TIME 1000
#endif

// Maximum time to wait for the network interface to accept a downlink message
// (milliseconds)
...
typedef struct LonUplinkStats {
  ...
  size_t rx_code_packet_failures; // Uplink CpFail received (implies downlink
                                  // checksum error)
...
// USB RX staging statistics structure; tracks ring-buffer receive staging
// activity
...
typedef struct LonStats {
  ...
  LonUsbInterfaceMode
      l2_l5_mode;      // Reported on NI reset and by MIP status response
  ...
  int ni_status;         // LON interface status reported by MIP status response
                 // 0 is ready; -1 is unknown; other values are LON interface
                 // specific
  OsalTickCount last_reset_time; // Timestamp of the last detected reset
                                 // condition on the LON interface
  OsalTickCount startup_time; // Initialization time for the most recent reset
  LonWatch runtime;           // Runtime since the most recent reset
} LonStats;

// Increment 'x' and peg at 0xFFFFFFFF on overflow
#define Increment32(x)                                                         \
  {                                                                            \
    if (x != 0xFFFFFFFF)                                                       \
      x++;                                                                     \
  }
...
typedef enum {
  ...
  DOWNLINK_WAIT_STARTUP_RESYNC_ACK, // Wait for uplink null frame indicating LON
                                    // interface restart after startup resync
  DOWNLINK_WAIT_RESYNC_RESET, // Wait for uplink reset message after startup
                              // resync
  ...
  DOWNLINK_NI_UNIQUE_ID_REQUEST, // Send read memory request for the LON
                                 // interface unique ID
  DOWNLINK_WAIT_NI_UNIQUE_ID,    // Wait for uplink memory read response with
                                 // unique ID
  ...
  DOWNLINK_WAIT_MIP_STATUS, // Wait for uplink MIP status response after sending
                            // MIP status request
  /* --- NORMAL OPERATIONS --- */
...
typedef enum {
  ...
  UPLINK_CODE_PACKET_CHECKSUM, // Waiting for code packet checksum byte; N/A for
                               // MIP/U61
...
#define UPLINK_BUF_LEN                                                         \
  ((MAX_LON_MSG_EX_LEN + 4) *                                                  \
   2) // Size of ul serial buffer (including expansions)
#define DOWNLINK_BUF_LEN                                                       \
  ((MAX_LON_MSG_EX_LEN + 4) *                                                  \
   2) // Size of dl serial buffer (including expansions)
...
typedef struct LonUsbQueueBuffer {
  ...
  LonFrameHeader frame_header;   // Two (frame sync and zero) or four-byte (code
                                 // packet) frame header
...
#if LINK_IS(USB_MIP)
typedef enum { U60_FT } LonUsbIfaceModel;
...
// Special sequence numbers for LonUsbLinkState.downlink_ack_seq_number to
...
__attribute__((used)) static LonUsbIfaceConfig lon_usb_iface_configs[] = {
    ...
    {LON_USB_INTERFACE_U50, LON_USB_LDISC_MIP_U50, FRAME_CODE_PACKET, true,
     true, true}};
#elif LINK_IS(MULTIPLE_USB_MIPS)
static LonUsbIfaceConfig lon_usb_iface_configs[] = {
    // U10 FT Rev A/B
    {LON_USB_INTERFACE_U61, LON_USB_LDISC_MIP_U61, FRAME_SYNC_ONLY, false,
     false, false},
    // U10 FT Rev C
    {LON_USB_INTERFACE_U50, LON_USB_LDISC_MIP_U50, FRAME_CODE_PACKET, true,
     true, true},
...
typedef struct LonUsbLinkState {
  ...
  bool wait_for_reset;    // True if waiting for reset command to be received on
                          // uplink
  ...
  // LON USB interface type (MIP/U50 vs MIP/U61)
  ...
  OsalTickCount
      resync_delay_timer;       // Resynchronization delay timer to synchronize
  ...
  LonUsbQueueBuffer uplink_buffer; // Current uplink buffer being built from the
                                   // USB uplink ring buffer
  int uplink_msg_index;            // Index into ((uint8_t
  ...
  bool uplink_tail_escaped; // True if last byte of a message was a frame sync
                            // byte
  OsalTickCount
      uplink_ack_timer; // Start time in ms ticks if waiting for an uplink ack
  ...
  int uplink_ack_timeout_phase;     // Uplink ack timeout phase, incremented on
                                    // repeated timeouts
  ...
  OsalTickCount
      downlink_reject_timer; // Downlink reject timeout timer;
  ...
  LonUsbQueueBuffer
      downlink_buffer; // Current downlink buffer from the head of the downlink
  ...
  int downlink_ack_seq_number; // Downlink sequence number to be acknowledged
                               ...
                               // Set to ACK_WITH_NEXT_SEQ_NUM if ack is
                               // required with the next sequence number; used
                               // for both code packet and message acks

  // Buffer queues for parsed messages; each queue holds LonUsbQueueBuffer
  // entries Implemented using the generic Queue type from lcs_queue.h
  ...
  // Uplink ring buffer for staging raw bytes received from the LON USB
  // interface before parsing into messages
  RingBuffer lon_usb_uplink_ring_buffer;
} LonUsbLinkState;

// Verify packed layout for 4-byte LonUsbFrameHeaderType
LON_STATIC_ASSERT(sizeof(LonUsbFrameHeaderType) == 4 * sizeof(uint8_t),
                  "LonFrameHeader size mismatch");

// IPV4 ICMP "poll" (ping) definitions -
//    ============================================================================
//    | 8                | 8                | 16 |
//    ============================================================================
//  0 | Version/IHL      | Type of service  | Length | 4 | Identification |
//  Flags & offset                     | 8 | TTL              | Protocol |
//  Header Checksum                    |
// 12 | Source IP address | 16 | Destination IP address |
//    ==ICMP
//    Header===============================================================
...
/*
 * Reads an uplink message from the LON USB network interface, if available.
 * Parameters:
 *   iface_index: interface index returned by OpenLonUsbLink()
 *             (multiple USB MIPS only)
