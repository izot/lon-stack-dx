...
// Expanded network management command codes for IUP
...
// Time required to initialize the image update process
#define IUP_INIT_IMAGE_UPDATE_INIT_TIMER 10  //seconds

// Result codes for Confirm Request
...
// Result codes for validate request
...
// Result codes for switch over request
...
// Result codes for commit request
...
typedef IZOT_STRUCT_BEGIN(IUP_ImageIdentifier)
{
    ...
    IzotByte imgInstance
            [8];  //	8-byte number that specifies the image within this scope
}
...
typedef IZOT_ENUM_BEGIN(TransferAckResponse){TRANSFER_STOP = 0,
        TRANSFER_CONTINUE = 1} IZOT_ENUM_END(TransferAckResponse);
