...
#include "abstraction/IzotEndian.h"  // IWYU pragma: keep
...
#ifndef _LCS_QUEUE_H
#include "lcs/lcs_queue.h"  // IWYU pragma: keep
#endif
#ifndef _TIMER_H
...
typedef struct __attribute__((__packed__)) {
    ...
    IzotByte subnet;
    BITS2(cloneDomain, 1, node, 7)
    BITS4(invalid, 1, unused, 2, authType,
            2,  // 1 => OMA (0 and 3 mean standard - the 3 is for backward compatibility with tools that wrote all ones to the domain length byte)
            len, 3)
} LogicalAddress;
...
typedef struct __attribute__((__packed__)) {
    BITS4(collisionDetect, 1, bitSyncThreshHold, 2, /* Encoded. See page 9-28 */
            filter, 2, hysteresis, 3)
    BITS3(unUsed, 6, cdTail, 1, cdPreamble, 1)
} DirectParamStruct;
...
typedef struct __attribute__((__packed__)) {
    ...
    BITS4(nvTurnaround, 1,   /* 1 ==> bound to a nv in this node */
            nvService,
            2, /* IzotServiceAcknowledged or UNACKD_RPT or IzotServiceUnacknowledged     */
...
typedef struct __attribute__((__packed__)) {
    ...
    IzotUbits16
            address_capacity; /* maximum number of address table entries available using
...
#define ALT_PATH                                                                         \
    0x01  // The traditional alt path bit - sent/received on the alternate carrier frequency (AKA secondary carrier)
#define ALT_RETRY 0x02  // This is a retry - downlink only
#define ALT_CHANNEL                                                                      \
    0x04  // Send/receive on the alternate chanenel if an alternate channel is available.
#define ALT_CHANNEL_LOCK                                                                 \
    0x08  // 1 => Transmission is locked to the primary/alternate channel based on previous bit.
...
typedef struct __attribute__((__packed__)) {
    ...
    BITS2(unused, 6, version, 2) /* version */
} NWSendParam;
...
typedef enum { TRANS_CURRENT, TRANS_NOT_CURRENT, TRANS_NEW, TRANS_DUPLICATE } TransStatus;
...
typedef struct __attribute__((__packed__)) {
    ...
    IzotByte proxyCount;  // Original proxy hop count.
    IzotUbits16
            txTimerDeltaLast;  // Amount to add to the last retry timer.  Only valid for proxy
...
typedef struct __attribute__((__packed__)) {
    BITS8(extRec, 1, nvSync, 1, nvPolled, 1, nvOffline, 1, nvServiceConfig, 1,
            nvPriorityConfig, 1, nvAuthConfig, 1, nvConfigClass, 1)
    IzotByte snvtTypeIndex;
} SNVTdescStruct;

typedef struct __attribute__((__packed__)) {
    BITS3(bindingII, 1, queryStats, 1, aliasCount, 6)
    IzotUbits16
            hostAlias;  // Warning - stored in host order so not for internal consumption
} AliasField;
...
#if LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS) || LINK_IS(SPI_MIP)
...
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS) || LINK_IS(SPI_MIP)
...
typedef struct {
    IzotUbits16 rx
            [2][2]
            [NUM_RX_TYPES];  // Alt path is first dimension, solicited/unsolicited is second
} RxStats;
...
#define IZOT_DATAPOINT_CHANGEABLE_TYPE_MASK                                              \
    0x02 /* Added for to store changeble type flag */
...
IzotByte IsGroupMember(IzotByte domainIndex, IzotByte groupIn, IzotByte *groupMemberOut);
...
IzotByte NodeUnConfigured(
        void);  // Node is not running and not honoring configuration (not necessarily the same as !NodeConfigured())
...
extern IzotBool IzotFilterMsgArrived(const IzotReceiveAddress *const pAddress,
        const IzotCorrelator correlator, const IzotBool priority,
        const IzotServiceType serviceType, const IzotBool authenticated,
        const IzotByte code, const IzotByte *const pData, const unsigned dataLength);
extern IzotBool IzotFilterResponseArrived(const IzotResponseAddress *const pAddress,
        const unsigned tag, const IzotByte code, const IzotByte *const pData,
        const unsigned dataLength);
