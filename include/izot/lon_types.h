...
typedef enum {
    ...
    LonStatusOutputBufferSizeMismatch =
            40,  // Priority and non-priority output buffer sizes must be the same
    LonStatusInvalidBufferParameter = 41,  // Invalid buffer parameter (e.g. too large)
    LonStatusInvalidBufferCount =
            42,  // Invalid buffer count (e.g. need at least one buffer of each type)
    LonStatusPriorityBufferCountMismatch =
            43,  // If one of the priority output buffer counts is zero, then both must be zero
    LonStatusBufferSizeTooSmall =
            44,  // Buffer size is too small to support subsequent buffer configuration changes
    LonStatusBufferConfigurationTooLarge =
            45,  // Requested buffer configuration is too large to fit in available space
    LonStatusAppBufferSizeMismatchWarning =
            46,  // Application buffer input-output size mismatch may cause problems (warning only)
    ...
    LonStatusSemaphoreError =
            65,                // Generic error creating or accessing a binary semaphore
    ...
    LonStatusDpPollNotPolledDatapoint =
            73,  // Polled attribute missing for polling input datapoint
    ...
    LonStatusLniNotDefined =
            89,  // No Network Interface defined. See IzotGetMyNetworkInterface in IzoTHandlers.c
    LonStatusLniNotResponding = 90,  // LON network interface not responding
    LonStatusLniUniqueIdNotAvailable =
            91,                     // LON network interface unique ID not available
    ...
    LonStatusRxMessageNotAvailable =
            94,  // No message received from the LON network interface

    // LON stack-related codes
    LonStatusLlpVersionNotAvailable =
            95,  // Link-layer protocol version information not available
    LonStatusDeviceUniqueIdNotAvailable =
            96,                         // Device unique ID (Neuron or MAC ID) unavailable
    ...
    LonStatusCallbackExceptionError =
            106,  // An exception when executing a callback function
    ...
    LonStatusDmfAddressOutOfRange =
            115,                 // DMF address + count is out of range for operation
    ...
    LonStatusIupImageWriteFailure =
            124,                        // Failed to write IUP image to persistent memory
...
#define IZOT_SET_UNSIGNED_WORD(n, v)                                                     \
    (n).msb = (IzotByte)((v) >> 8);                                                      \
    (n).lsb = (IzotByte)(v)
#define IZOT_SET_UNSIGNED_WORD_FROM_BYTES(n, b1, b2)                                     \
    (n).msb = (IzotByte)(b1);                                                            \
    (n).lsb = (IzotByte)(b2)

#define IZOT_GET_SIGNED_WORD(n) ((int16_t)IZOT_GET_UNSIGNED_WORD(n))
#define IZOT_SET_SIGNED_WORD(n, v) IZOT_SET_UNSIGNED_WORD(n, v)
#define IZOT_GET_UNSIGNED_DOUBLEWORD(n)                                                  \
    ((((uint32_t)IZOT_GET_UNSIGNED_WORD((n).msw)) << 16) +                               \
            (uint32_t)IZOT_GET_UNSIGNED_WORD((n).lsw))
#define IZOT_SET_UNSIGNED_DOUBLEWORD(n, v)                                               \
    IZOT_SET_UNSIGNED_WORD((n).msw, (uint16_t)((v) >> 16));                              \
...
#define IZOT_SET_ATTRIBUTE(var, n, value)                                                \
    ((var).n##_FIELD = (((var).n##_FIELD) & ~n##_MASK) |                                 \
                       ((((value) << n##_SHIFT)) & n##_MASK))
#define IZOT_SET_ATTRIBUTE_P(var, n, value)                                              \
    ((var)->n##_FIELD = (((var)->n##_FIELD) & ~n##_MASK) |                               \
                        ((((value) << n##_SHIFT)) & n##_MASK))
...
#define IZOT_NODE_STATE(state)                                                           \
    ((IZOT_NEURON_STATE(state) == IzotConfigOnLine) ? (state) : IZOT_NEURON_STATE(state))

// Extracts offline details from <IzotNeuronState>.
// Use this macro to query the node's offline modes.
#define IZOT_NODE_STATE_OFFLINE(state)                                                   \
    (IZOT_NEURON_STATE(state) == IzotConfigOffLine ||                                    \
            IZOT_NEURON_STATE(state) == IzotSoftOffLine)
...
typedef IZOT_ENUM_BEGIN(IzotNodeState){
        ...
        /* 12 */ IzotSoftOffLine =
                (IzotConfigOnLine |
                        IZOT_OFFLINE_BIT), /* (12) configured, soft-offline        */
...
typedef IZOT_ENUM_BEGIN(IzotNodeMode){IzotApplicationOffLine = 0,
        IzotApplicationOnLine = 1, IzotApplicationReset = 2, IzotChangeState = 3,
        IzotPhysicalReset = 6} IZOT_ENUM_END(IzotNodeMode);

// Decodes the last reset cause
typedef IZOT_ENUM_BEGIN(IzotResetCause){/* 0x00 */ IzotResetCleared = 0x00,
        /* 0x01 */ IzotPowerUpReset = 0x01,
        /* 0x02 */ IzotExternalReset = 0x02,
        /* 0x0C */ IzotWatchdogReset = 0x0C,
        /* 0x14 */ IzotSoftwareReset = 0x14} IZOT_ENUM_END(IzotResetCause);
...
typedef IZOT_ENUM_BEGIN(IzotAddressType){/*  0 */ IzotAddressUnassigned = 0,
        /*  1 */ IzotAddressSubnetNode,
        /*  2 */ IzotAddressUniqueId,
        /*  3 */ IzotAddressBroadcast,
        /*127 */ IzotAddressLocal = 127} IZOT_ENUM_END(IzotAddressType);
...
#ifndef _LTDRIVER_H
// Service LED states
typedef IZOT_ENUM_BEGIN(IzotServiceLedState){SERVICE_OFF = 0, SERVICE_ON = 1,
        SERVICE_BLINKING = 2,

        // Software controlled only
        SERVICE_FLICKER = -1} IZOT_ENUM_END(IzotServiceLedState);
...
#define NUM_ADDR_TBL_ENTRIES                                                             \
    254 /* # of address table entries; maximum supported value is 255 */
...
#define NV_ALIAS_TABLE_SIZE                                                              \
    254 /* Check management tool for any restriction on maximum size */
...
*******************************************************************************/
#if 0
// TODO: make this conditional based on malloc support in OSAL
#ifndef MALLOC_SIZE
...
#define IZOT_SENDGROUP_TYPE_MASK                                                         \
    0x80 /* One for group, zero for all other address types */
...
#define IZOT_SENDGROUP_MEMBER_MASK                                                       \
    0x7F /* member ID within the group (0-63). Use zero for unlimited groups. */
...
#define IZOT_SENDGROUP_REPEAT_TIMER_MASK                                                 \
    0xF0 /* repeat timer. Use values from the <IzotRepeatTimer> enumeration. */
...
#define IZOT_SENDGROUP_RETRY_MASK                                                        \
    0x0F /* number of retries, or number of transmissions minus one for unacknowledged service */
...
#define IZOT_SENDGROUP_RECEIVE_TIMER_MASK                                                \
    0xF0 /* receive timer. Use values from the <IzotReceiveTimer> enumeration. */
...
#define IZOT_SENDGROUP_TRANSMIT_TIMER_MASK                                               \
    0x0F /* receive timer. Use values from the <IzotTransmitTimer> enumeration. */
...
{
    IzotByte
            TypeSize; /* contains type, size. See IZOT_SENDGROUP_TYPE_* and _SIZE_* macros. */
    IzotByte
            DomainMember; /* contains domain, member. See IZOT_SENDGROUP_DOMAIN_* and _MEMBER_* macros. */
    IzotByte
            RepeatRetry; /* contains repeat, retry. See IZOT_SENDGROUP_REPEAT_* and _RETRY_* macros. */
    IzotByte
            ReceiveTransmit; /* contains receive and transmit timers. See IZOT_SENDGROUP_RECEIVE_* and _TRANSMIT_* macros. */
...
#define IZOT_SENDSN_REPEAT_TIMER_MASK                                                    \
    0xF0 /* repeat timer. Use values from the <IzotRepeatTimer> enumeration. */
...
#define IZOT_SENDSN_TRANSMIT_TIMER_MASK                                                  \
    0x0F /* Transmit timer. Use values from the <IzotTransmitTimer> enumeration. */
...
{
    IZOT_ENUM(IzotAddressType)
    Type; /* Set to IzotAddressSubnetNode for subnet/node addressing */
    IzotByte
            DomainNode; /* Contains domain, node. See IZOT_SENDSN_DOMAIN_* and _NODE_* macros. */
    IzotByte
            RepeatRetry; /* Contains repeat, retry. See IZOT_SENDSN_REPEAT_* and _RETRY_* macros. */
    IzotByte
            RsvdTransmit; /* Contains rsvd0, transmit. See IZOT_SENDSN_RSVD0_* and _TRANSMIT_TIMER_* macros. */
...
#define IZOT_SENDNID_REPEAT_TIMER_MASK                                                   \
    0xF0 /* repeat timer. Use values from the <IzotRepeatTimer> enumeration. */
...
#define IZOT_SENDNID_TRANSMIT_TIMER_MASK                                                 \
    0x0F /* Transmit timer. Use values from the <IzotTransmitTimer> enumeration. */
...
typedef IZOT_STRUCT_BEGIN(IzotSendUniqueId)
{
    ...
    IzotByte
            Domain; /* Contains the domain index. See IZOT_SENDNID_DOMAIN_* macro. The remaining 7 bits must be zero. */
    IzotByte
            RepeatRetry; /* Contains repeat, retry. See IZOT_SENDNID_REPEAT_* and _RETRY_* macros. */
    IzotByte
            RsvdTransmit; /* Contains rsvd0, transmit. See IZOT_SENDNID_RSVD0_* and _TRANSMIT_TIMER_* macros. */
    IzotSubnetId
            Subnet; /* Destination subnet number, 1..255, or zero to pass all routers */
...
#define IZOT_SENDBCAST_REPEAT_TIMER_MASK                                                 \
    0xF0 /* Repeat timer. Use values from the <IzotRepeatTimer> enumeration. */
...
#define IZOT_SENDBCAST_TRANSMIT_TIMER_MASK                                               \
    0x0F /* Transmit timer. Use values from the <IzotTransmitTimer> enumeration. */
...
typedef IZOT_STRUCT_BEGIN(IzotSendBroadcast)
{
    ...
    IzotByte
            DomainRsvdBacklog; /* Contains domain, rsvd0, backlog. See IZOT_SENDBCAST_DOMAIN_*, _RSVD0_* and _BACKLOG_* macros. */
    IzotByte
            RepeatRetry; /* Contains repeat, retry. See IZOT_SENDBCAST_REPEAT_* and _RETRY_* macros. */
    IzotByte RsvdTransmit;
    IzotSubnetId
            Subnet; /* Destination subnet number, 1..255 for subnet broadcast, zero for domain broadcast */
}
...
typedef IZOT_STRUCT_BEGIN(IzotReceiveGroup) { IzotGroupId GroupId; /* 0..255 */ }
...
typedef IZOT_ENUM_BEGIN(IzotReceiveDestinationAddressFormat){
        ...
        /*  4 */
        IzotReceiveDestinationAddressTurnaround} IZOT_ENUM_END(IzotReceiveDestinationAddressFormat);
...
#define IZOT_RECEIVEADDRESS_DOMAIN_MASK                                                  \
    0x80 /* Domain table index through which the message was received */
#define IZOT_RECEIVEADDRESS_DOMAIN_SHIFT 7
#define IZOT_RECEIVEADDRESS_DOMAIN_FIELD DomainFormat

#define IZOT_RECEIVEADDRESS_FLEX_MASK                                                    \
    0x40 /* One for flex domain, that is, received message on unconfigured node */
#define IZOT_RECEIVEADDRESS_FLEX_SHIFT 6
#define IZOT_RECEIVEADDRESS_FLEX_FIELD DomainFormat

#define IZOT_RECEIVEADDRESS_FORMAT_MASK                                                  \
    0x3F /* Use <IzotReceiveDestinationAddressFormat> enumeration */
...
{
    IzotByte
            DomainFormat; /* Contains domain, flex domain, format. Use IZOT_RECEIVEADDRESS_* macros to access data. */
...
#define IZOT_RESPONSESOURCE_IS_SUBNETNODE_MASK                                           \
    0x80 /* 1: subnet/node response. 0: group response. */
...
typedef IZOT_STRUCT_BEGIN(IzotResponseSource)
{
    ...
    IzotByte
            Node; /* contains node, isSubnetNode. Use IZOT_RESPONSESOURCE_NODE_* and IZOT_RESPONSESOURCE_IS_SUBNETNODE_* macros. */
}
...
{
    IzotByte
            Domain; /* contains domain, flex domain. Use IZOT_RESPONSEADDRESS_* macros. */
...
#define IZOT_ADDRESS_GROUP_RECEIVE_TIMER_MASK                                            \
    0xF0 /* Receive timer, use IzotReceiveTimer */
#define IZOT_ADDRESS_GROUP_RECEIVE_TIMER_SHIFT 4
#define IZOT_ADDRESS_GROUP_RECEIVE_TIMER_FIELD ReceiveTransmit

#define IZOT_ADDRESS_GROUP_TRANSMIT_TIMER_MASK                                           \
    0x0F /* Transmit timer, use IzotTransmitTimer */
...
typedef IZOT_STRUCT_BEGIN(IzotAddressTableGroup)
{
    ...
    IzotByte
            DomainMember; /* Contains domain, member. Use the IZOT_ADDRESS_GROUP_* macros. */
    IzotByte
            RepeatRetry; /* Contains repeatTimer, retry. Use the IZOT_ADDRESS_GROUP_* macros. */
    IzotByte
            ReceiveTransmit; /* Contains receive and transmit timer. Use Izot_ADDRESS_GROUP_* macros. */
...
#define IZOT_ADDRESS_BROADCAST_REPEAT_TIMER_MASK                                         \
    0xF0 /* Repeat timer, use IzotRepeatTimer */
...
typedef IZOT_STRUCT_BEGIN(IzotAddressTableBroadcast)
{
    ...
    IzotByte
            RepeatRetry; /* Repeat timer and retry. Use IZOT_ADDRESS_BROADCAST_* macros instead. */
    IZOT_ENUM(IzotTransmitTimer) TransmitTimer;
    IzotSubnetId Subnet;
}
...
typedef IZOT_STRUCT_BEGIN(IzotAddressTableTurnaround)
{
    ...
    IzotByte
            RepeatRetry; /* Contains repeat timer and retry. Use IZOT_ADDRESS_TURNAROUND_* macros. */
    IZOT_ENUM(IzotTransmitTimer) TransmitTimer;
}
...
typedef IZOT_ENUM_BEGIN(IzotDomainLength){/*  0 */ IzotDomainLength_0 = 0,
        /*  1 */ IzotDomainLength_1 = 1,
        /*  3 */ IzotDomainLength_3 = 3,
        /*  6 */ IzotDomainLength_6 = 6} IZOT_ENUM_END(IzotDomainLength);
...
typedef IZOT_STRUCT_BEGIN(IzotDomain)
{
    ...
    IzotByte
            InvalidIdLength; /* use IZOT_DOMAIN_INVALID_* and IZOT_DOMAIN_ID_LENGTH_* macros */
    IzotAuthenticationKey Key;
}
IZOT_STRUCT_END(IzotDomain);

// Encodes the direction of a datapoint
typedef IZOT_ENUM_BEGIN(
        IzotDatapointDirection){/*  0 */ IzotDatapointDirectionIsInput = 0,
        /*  1 */ IzotDatapointDirectionIsOutput =
                1} IZOT_ENUM_END(IzotDatapointDirection);
...
{
    IzotByte
            SelhiDirPrio; /* Contains selector-high, direction, priority. Use IZOT_DATAPOINT_* macros. */
    IzotByte SelectorLow; /* Contains selector-low. Use IZOT_DATAPOINT_* macros. */
    IzotByte
            Attribute1; /* Contains turnaround, service, authentication, and address-low. Use IZOT_DATAPOINT_* macros. */
...
{
    IzotByte
            EcsSelhiDirPrio; /* selector-high, direction, priority. Use IZOT_DATAPOINT_ECS_* macros. */
    IzotByte SelectorLow;
    IzotByte
            Attributes1; /* turnaround, authentication write-by-index, remote-nm-auth, response-selection. Use IZOT_DATAPOINT_* macros. */
    IzotByte
            Attributes2; /* read-by-index, service, request-selection, update-selection, source-selection-only. Use IZOT_DATAPOINT_ECS_* macros. */
...
{
    IzotByte
            Parameter_1; /* Collision-detect, bit-sync-threshold, filter, and hysteresis. Use IZOT_DIRECT_XCVR_* macros. */
    IzotByte
            Parameter_2; /* cd-to-end-packet, cd-tail, cd-preamble. Use IZOT_DIRECT_XCVR_* macros. */
}
...
#define IZOT_DATAPOINT_OFFLINE                                                           \
    0x00000010 /* Network tools should only change this value when the device is offline. */
#define IZOT_DATAPOINT_POLLED                                                            \
    0x00000020 /* This is either a polling input or a polled output. */
...
#define IZOT_DATAPOINT_UNACKD_RPT                                                        \
    0x00000800                           /* Default to unacknowledged repeat service. */
...
#define IZOT_CONFIG_NONGRPRCV_MASK                                                       \
    0xF0 /* Non-group receive timer, use <IzotNonGroupReceiveTimer> */
...
typedef IZOT_STRUCT_BEGIN(IzotConfigData)
{
    ...
    IzotByte
            CommConfiguration; /* Communications type and communications pin direction; use IZOT_CONFIG_* macros */
    ...
    IzotByte
            Config_1; /* Pre-emption timeout, network management authentication, non-group receive timer; use IZOT_CONFIG_* macros */
}
...
{
    IzotUniqueId
            UniqueNodeId; /* 48-bit unique ID for the device                                   */
    IzotByte
            ArchNum; /* LON architecture; set to <LonArchitecture> value                  */
    IzotByte
            CheckSumMinorNum; /* Checksum for Unique Node ID and minorArchNum (0-128); use IZOT_READONLY_* values */
    IzotByte DatapointFixed
            [2]; /* Location of NV fixed table                                        */
    IzotByte
            ReadOnly_1; /* Contains:  (Use IZOT_READONLY_* values)                           */
    /* readWriteProtect , 1,       Read+write protect flag               */
    /* runWhenUnconf    , 1,       1 => Application runs when unconfigured */
    /* nvCount          , 6,                                             */
    IzotByte SnvtStruct
            [2]; /* 0xFFFF for LON Stack                                              */
    IzotProgramId
            ProgramId; /* Program ID string (array of 8 bytes)                              */
    IzotByte
            ReadOnly_2; /* Contains:  (Use IZOT_READONLY_* values)                           */
    ...
    IzotByte
            ReadOnly_3; /* Contains: (Use IZOT_READONLY_* values)                            */
    /* # of entries in address table (AddressCn)                         */
    /* Reserved 5       , 4        Unused                                */
    IzotByte
            ReadOnly_4; /* Contains:  (Use IZOT_READONLY_* values)                           */
    /* Reserved 6       , 4,       Unused                                */
    /* receiveTransCnt  , 4        RR Cnt = this field + 1               */
    IzotByte
            AppBufSize; /* appOutBufSize    , 4,       Special size encoding                 */
    /* appInBufSize     , 4        Special size encoding                 */
    IzotByte
            NwBufSize; /* nwOutBufSize     , 4,       Special size encoding                 */
    /* nwInBufSize      , 4        Special size encoding                 */
    IzotByte
            PriCnt; /* nwOutBufPriCnt   , 4,       Special count encoding                */
    /* appOutBufPriCnt  , 4        Special count encoding                */
    IzotByte
            AppBufCnt; /* appOutBufCnt     , 4,       Special count encoding                */
    /* appInBufCnt      , 4        Special count encoding                */
    IzotByte
            NwBufCnt; /* nwOutBufCnt      , 4,       Special count encoding                */
    ...
    IzotByte Reserved2
            [3]; /* Unused                                                            */
    IzotByte
            ReadOnly_5; /* Contains:  (Use IZOT_READONLY_* values)                           */
    /* Reserved 7       , 6,       Unused                                */
    /* txByAddress      , 1,       0 in LON Stack                        */
    /* Reserved 8       , 1)       Unused                                */
    IzotByte
            ReadOnly_6; /* Reserved 9       , 2,       Unused                                */
    /* aliasCnt         , 6)       0 in LON Stack                        */
    IzotByte
            ReadOnly_7; /* msgTagCnt        , 4,       0 in LON Stack                        */
    ...
    IzotByte Snvt2Lo;
    IzotByte
            ReadOnly_8; /* Direct memory file , 1,                                           */
    /* r11              , 7                                              */
...
typedef IZOT_ENUM_BEGIN(
        LonNiCommand){LonNiClearCmd = 0x00,  // Clear (no operation)--used by driver
        ...
        LonNiDiagnosticData =
                0x13,  // Diagnostic data used for debugging, testing, and statistics
        ...
        LonNiLocalNetMgmtCmd =
                0x22,  // Local network management command--used by stack L2 and driver
        ...
        LonNiOnlineCmd = 0x70, LonNiOfflineCmd = 0x80, LonNiFlushCmd = 0x90,
        LonNiFlushIgnoreCmd = 0xA0, LonNiSleepCmd = 0xB0,
        ...
        LonNiFlowControlCmd = 0xD5, LonNiResetDriverCmd = 0xD6,
        LonNiCycleDriverCmd = 0xD7,
        LonNiStatusCmd = 0xE0,  // Used by driver
        LonNiPupXoffCmd = 0xE1, LonNiPupXonCmd = 0xE2, LonNiPtrHRotlCmd = 0xE4,
        LonNiLayerModeCmd = 0xE5,  // Layer mode in payload--used by driver
        LonNiRqEnableCmd = 0xE5, LonNiTxIdCmd = 0xE8,
        LonNiSltaPlsCmd = 0xEA,  // SLTA only
        LonNiNonLonCmd = 0xEF, LonNiDriverCmd = 0xF0} IZOT_ENUM_END(LonNiCommand);
...
typedef IZOT_ENUM_BEGIN(IzotNmMessageCode){// Network diagnostic commands
        IzotNdQueryStatus = 0x51, IzotNdProxy = 0x52, IzotNdClearStatus = 0x53,
        IzotNdQueryXcvr = 0x54, IzotNdQueryStatusFlexDomain = 0x56,

        // Network management commands
        IzotNmExpanded = 0x60, IzotNmQueryId = 0x61, IzotNmRespondToQuery = 0x62,
        IzotNmUpdateDomain = 0x63, IzotNmLeaveDomain = 0x64, IzotNmUpdateKey = 0x65,
        IzotNmUpdateAddr = 0x66, IzotNmQueryAddr = 0x67,
        IzotNmQueryDatapointConfig = 0x68, IzotNmUpdateGroupAddr = 0x69,
        IzotNmQueryDomain = 0x6A, IzotNmUpdateDatapointConfig = 0x6B,
        IzotNmSetNodeMode = 0x6C, IzotNmReadMemory = 0x6D, IzotNmWriteMemory = 0x6E,
        IzotNmChecksumRecalculation = 0x6F, IzotNmWink = 0x70,
        IzotNmInstall = IzotNmWink, /* See <IzotInstallCommand> */
        IzotNmAppCommand = IzotNmWink, IzotNmMemoryRefresh = 0x71, IzotNmQuerySnvt = 0x72,
        IzotNmDatapointFetch = 0x73, IzotNmDeviceEscape = 0x7D, IzotNmRouterEscape = 0x7E,
        IzotNmServicePin = 0x7F} IZOT_ENUM_END(IzotNmMessageCode);
...
typedef IZOT_ENUM_BEGIN(IzotDatapointInfoType){
        IzotDatapointInfoDescriptor =
                0, /* Query datapoint description (IzotNmInstallResponse.DatapointDescriptor) */
        IzotDatapointInfoRateEstimate =
                1, /* Query datapoint rate estimates (IzotNmInstallResponse.DatapointRate) */
        IzotDatapointInfoName =
                2, /* Query datapoint Name (IzotNmInstallResponse.DatapointName) */
        IzotDatapointInfoSdText =
                3, /* Query datapoint self-documentation string (IzotNmInstallResponse.DatapointSd) */
        IzotDatapointInfoSnvtIndex =
                4 /* Query datapoint SNVT index (IzotNmInstallResponse.SnvtTypeIndex) */
...
typedef IZOT_ENUM_BEGIN(IzotNodeInfoType){
        IzotNodeInfoSdText =
                3, /* Query node self-documentation string (IzotNmInstallResponse.NodeSd) */
...
typedef IZOT_UNION_BEGIN(IzotNmInstallRequest)
{
    ...
    IZOT_STRUCT_NESTED_BEGIN(QueryDatapointInfo)
    {
        ...
        IZOT_ENUM(IzotDatapointInfoType)
        DatapointInfoType;       /* Requested datapoint information */
...
#define IZOT_DATAPOINT_DESC_DFLT_SERVICE_MASK                                            \
    0x03 /* Default service type; use IzotServiceType. */
...
#define IZOT_DATAPOINT_DESC_ATTR_SYNC_MASK                                               \
    0x40 /* 1: sync Datapoint, 0: non-sync Datapoint */
...
#define IZOT_DATAPOINT_DESC_ATTR_POLLED_MASK                                             \
    0x20 /* 1: polled or polling input, 0: not polled */
...
#define IZOT_DATAPOINT_DESC_ATTR_SRVC_TYPE_CONFIG_MASK                                   \
    0x08 /* 1: service type is configurable */
...
#define IZOT_DATAPOINT_DESC_ATTR_PRIORITY_CONFIG_MASK                                    \
    0x04 /* 1: priority flag is configurable */
...
#define IZOT_DATAPOINT_DESC_ATTR_AUTH_CONFIG_MASK                                        \
    0x02 /* 1: authentication flag is configurable */
...
#define IZOT_DATAPOINT_DESC_ATTR_CONFIG_CLASS_MASK                                       \
    0x01 /* 1: a config Datapoint, 0: non-config Datapoint */
...
#define IZOT_DATAPOINT_DESC_EXT_ATTR_MRE_AVAIL_MASK                                      \
    0x80 /* 1: Max rate estimate available using IzotInstallQueryDatapointInfo, IzotDatapointInfoRateEstimate */
...
#define IZOT_DATAPOINT_DESC_EXT_ATTR_RE_AVAIL_MASK                                       \
    0x40 /* 1: Average rate estimate available using IzotInstallQueryDatapointInfo, IzotDatapointInfoRateEstimate */
...
#define IZOT_DATAPOINT_DESC_EXT_ATTR_NAME_AVAIL_MASK                                     \
    0x20 /* 1: Datapoint Name is available using IzotInstallQueryDatapointInfo, IzotDatapointInfoName. */
...
#define IZOT_DATAPOINT_DESC_EXT_ATTR_SD_AVAIL_MASK                                       \
    0x10 /* 1: Datapoint SD text is available using IzotInstallQueryDatapointInfo, IzotDatapointInfoSdText  */
...
#define IZOT_DATAPOINT_DESC_EXT_ATTR_NAME_SUPPLIED_MASK                                  \
    0x08 /* 1: Name is supplied in IzotNmInstallResponse.DatapointDescriptor */
...
typedef IZOT_UNION_BEGIN(IzotNmInstallResponse)
{
    ...
    {
        IzotByte
                LengthAndOrigin; /* Use IZOT_DATAPOINT_DESC_LENGTH_* and IZOT_DATAPOINT_DESC_ORIGIN_* macros */
        IzotByte
                Defaults; /* Use IZOT_DATAPOINT_DESC_IS_OUTPUT_* and IZOT_DATAPOINT_DESC_DFLT_* macros */
    ...
    {
        IzotByte
                RateEstimate; /* Encoded rate estimate. Only valid if 'IZOT_DATAPOINT_DESC_EXT_ATTR_RE_AVAIL' is set in DatapointDescriptor. */
        IzotByte
                MaxRateEstimate; /* Encoded max rate estimate; only valid if 'IZOT_DATAPOINT_DESC_EXT_ATTR_MRE_AVAIL' is set in DatapointDescriptor. */
    }
    IZOT_STRUCT_NESTED_END(DatapointRate);

    /* Response for IzotInstallQueryDatapointInfo, IzotDatapointInfoName */
    char DatapointName
            [IZOT_DATAPOINT_NAME_LEN]; /* Datapoint name; only valid if 'nm' set in Datapoint DatapointDescriptor. */
...
typedef IZOT_ENUM_BEGIN(IzotMemoryReadWriteMode){
        ...
        IzotConfigStructRelative =
                2, /* Address is offset into configuration data structures */
        IzotStatisticStructRelative =
                3, /* Address is offset into statistics data structures */
...
typedef IZOT_ENUM_BEGIN(IzotMemoryWriteForm){IzotNoAction = 0,
        IzotBothCsRecalculation = 1, IzotDeltaCsRecalculation = 3,
        IzotConfigCsRecalculation = 4, IzotOnlyReset = 8,
        IzotBothCsRecalculationReset = 9,
        IzotConfigCsRecalculationReset = 12} IZOT_ENUM_END(IzotMemoryWriteForm);
...
typedef IZOT_ENUM_BEGIN(IzotApplicationMessageCode){IzotApplicationMsg = 0x00,
        IzotApplicationIsi = 0x3D, IzotApplicationFtp = 0x3E,
        IzotApplicationIsOffLine = 0x3F, IzotForeignMsg = 0x40,
        IzotForeignIsOffLine = 0x4F,
        IzotLastMessageCode = 0x4F} IZOT_ENUM_END(IzotApplicationMessageCode);
...
typedef IZOT_STRUCT_BEGIN(IzotNdQueryStatusResponse) { IzotStatus Status; }
IZOT_STRUCT_END(IzotNdQueryStatusResponse);

// Message structure used with responses to *IzotNdQueryXcvr*
typedef IZOT_STRUCT_BEGIN(IzotNdQueryXcvrResponse) { IzotTransceiverParameters Status; }
...
typedef IZOT_STRUCT_BEGIN(IzotNmUpdateDatapointRequest)
{
    ...
    {
        IZOT_STRUCT_NESTED_BEGIN(ShortForm) { IzotDatapointConfig DatapointConfig; }
...
typedef IZOT_STRUCT_BEGIN(IzotNmUpdateAliasRequest)
{
    ...
    IZOT_UNION_NESTED_BEGIN(Request)
    {
        IZOT_STRUCT_NESTED_BEGIN(ShortForm) { IzotAliasConfig AliasConfig; }
        IZOT_STRUCT_NESTED_END(ShortForm);
        IZOT_STRUCT_NESTED_BEGIN(LongForm)
        {
            IzotWord LongIndex;
...
typedef IZOT_STRUCT_BEGIN(IzotNmUpdateDomainResponse) { IzotDomain Domain; }
 ...
 */
typedef IzotPersistentSegType (*IzotPersistentSegOpenForReadFunction)(
 ...
 */
typedef LonStatusCode (
        *IzotPersistentSegReadFunction)(const IzotPersistentSegType persistent_seg_type,
 ...
 */
typedef LonStatusCode (
        *IzotPersistentSegWriteFunction)(const IzotPersistentSegType persistent_seg_type,
 ...
 *  returns TRUE.
 */
typedef IzotBool (*IzotPersistentSegIsInvalidFunction)(
        const IzotPersistentSegType persistent_seg_type);
 ...
 *  fails.
 */
typedef LonStatusCode (*IzotPersistentSegEnterTransactionFunction)(
        const IzotPersistentSegType persistent_seg_type);
 ...
 *  event. Without an application-specific handler, this event always fails.
 */
typedef LonStatusCode (*IzotPersistentSegExitTransactionFunction)(
        const IzotPersistentSegType persistent_seg_type);
 ...
 *  fails.
 */
typedef LonStatusCode (*IzotPersistentSegDeserializeFunction)(const void *const pData,
        const size_t size);
 ...
 */
typedef LonStatusCode (
        *IzotPersistentSegSerializeFunction)(void *const pData, const size_t size);
 ...
 *  event. Without an application-specific handler, this event does nothing.
 */
typedef void (*IzotDatapointUpdateOccurredFunction)(const unsigned index,
 ...
 *  event. Without an application-specific handler, this event does nothing.
 */
typedef void (*IzotDatapointUpdateCompletedFunction)(const unsigned index,
 ...
 */
typedef void (*IzotMsgArrivedFunction)(const IzotReceiveAddress *const pAddress,
 ...
 *  Without an application-specific handler, this event does nothing.
 */
typedef void (*IzotResponseArrivedFunction)(const IzotResponseAddress *const pAddress,
 ...
 *  Without an application-specific handler, this event does nothing.
 */
typedef void (*IzotMsgCompletedFunction)(const unsigned tag, const IzotBool success);
 ...
 *  Without an application-specific handler, this event does nothing.
 */
typedef void (*IzotServiceLedStatusFunction)(IzotServiceLedState state,
 ...
 */
typedef IzotBool (*IzotFilterMsgArrivedFunction)(const IzotReceiveAddress *const pAddress,
        const IzotCorrelator correlator, const IzotBool priority,
        const IzotServiceType serviceType, const IzotBool authenticated,
        const IzotByte code, const IzotByte *const pData, const unsigned dataLength);
...
typedef IzotBool (*IzotFilterResponseArrivedFunction)(
        const IzotResponseAddress *const pAddress, const unsigned tag,
        const IzotByte code, const IzotByte *const pData, const unsigned dataLength);
 ...
 */
typedef IzotBool (
        *IzotFilterMsgCompletedFunction)(const unsigned tag, const IzotBool success);
...
#undef _LON_TYPES_H_PARSING
#ifndef _IZOT_PLATFORM_NO_UMBRELLA
#ifndef _TIMER_H
#include "lcs/lcs_timer.h"  // IWYU pragma: export
#endif
#ifndef _LCS_API_H
#ifndef _NODE_H
#include "lcs/lcs_node.h"  // IWYU pragma: export
#endif
#endif
#ifndef _IZOT_OSAL_H
#include "abstraction/IzotOsal.h"  // IWYU pragma: export
#endif
#ifndef _IZOT_HAL_H
#include "abstraction/IzotHal.h"  // IWYU pragma: export
#endif
#ifndef _IZOT_CAL_H
#include "abstraction/IzotCal.h"  // IWYU pragma: export
#endif
#endif
