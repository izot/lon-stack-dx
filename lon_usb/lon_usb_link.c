 ...
 * 			  • lon_usb_uplink_ring_buffer (staging ring for raw bytes)
 * 			  • uplink_queue (singly-linked list of parsed messages)
 * 			- Producers:
 * 			  • ProcessUplinkBytes() (invoked by ReadLonUsbMsg) produces
 *              messages into uplink_queue
 * 			- Consumers:
 * 			  • ReadLonUsbMsg() drains lon_usb_uplink_ring_buffer and pops from
 *              uplink_queue to return a message to the caller
 * 			- Locking rules:
 * 			  • Always OsalLockQueue(&state->queue_lock) before reading/writing
 *              lon_usb_uplink_ring_buffer or uplink_queue, including related
 *              stats updates
 *            • Keep the critical section minimal; perform parsing and other
 *              CPU work outside the lock • For uplink_queue pop, copy the node’s
 *              payload inside the lock, then free the node after unlocking to
...
#define FRAME_CODE_SEQ_NUM_INC 0x20   // Sequence number increment (add to
                                      // current value; wrap at 7)
...
static const LonDataFrame MsgReportStatus[] = {
        LonNiLocalNetMgmtCmd,           // Network interface command
        UNICAST_MSG_CODE_OFFSET + (1),  // 15 PDU length
        0x6F,  // 0 msg_type, 3 st (request), 0 auth, 15 tag (private)
        0x08,  // 0 priority, 0 path, 0 2-bit cmpl_code, 1 addr_mode, 0 alt_path, 0
               // pool, 0 response
        (1),   // 1 message length
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00,  // Network address
        IzotNdQueryStatus};
...
static const LonDataFrame MsgUniqueId = {
        LonNiLocalNetMgmtCmd,           // Network interface command (0x22)
        UNICAST_MSG_CODE_OFFSET + (5),  // 19 PDU length
        {
                0x70,  // 0 msg_type, 3 2-bit st (request), 1 auth, 0 4-bit tag
                0x00,  // 0 priority, 0 path, 0 2-bit cmpl_code, 0 addr_mode, 0 alt_path,
                       // 0 pool, 0 response
                (5),   // 5 message length
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                0x00,        // Network address
...
bool linkInitialized = false;  // Flag to indicate if LON USB
                               // link is initialized
#if LINK_IS(USB_MIP)
LonUsbLinkState iface_state;  // Single interface state
#else                         // LINK_IS(MULTIPLE_USB_MIPS)
...
#if LINK_IS(USB_MIP)
static LonStatusCode StartLonUsbLink(LonUsbInterfaceMode configured_iface_mode);
static LonStatusCode ResetUplinkState(void);
static LonStatusCode RequestUsbNiUid(void);
static LonStatusCode RequestUsbNiStatus(void);
static LonStatusCode SetNiLayerMode(LonUsbInterfaceMode iface_mode);
#else   // LINK_IS(MULTIPLE_USB_MIPS)
static LonStatusCode StartLonUsbLink(int iface_index,
...
#if LINK_IS(USB_MIP)
static LonStatusCode ProcessDownlinkRequests(void);
static LonStatusCode ProcessNextDownlinkMessage(void);
static LonStatusCode WriteDownlinkMessage(void);
static LonStatusCode WriteDownlinkLocalNiCmd(void);
static LonStatusCode WriteDownlinkCodePacket(LonUsbFrameCommand frame_cmd,
        uint8_t parameter);
static LonStatusCode IncrementDownlinkSequence(void);
static LonStatusCode StartAckTimer(DownlinkState next_state);
static LonStatusCode ResetDownlinkState(void);
static LonStatusCode StopAckTimer(void);
#else   // LINK_IS(MULTIPLE_USB_MIPS)
static LonStatusCode ProcessDownlinkRequests(int iface_index);
...
static LonStatusCode WriteDownlinkCodePacket(int iface_index,
        LonUsbFrameCommand frame_cmd, uint8_t parameter);
...
static LonStatusCode CreateFrameHeader(LonFrameHeader *frame_header,
        uint8_t frame_command, uint8_t parameter, uint8_t sequence_num, bool ack);
static uint8_t ComputeChecksum(uint8_t *pkt, size_t len);

// LON uplink processing
#if LINK_IS(USB_MIP)
static LonStatusCode ProcessUplinkBytes(uint8_t *chunk, size_t chunk_size);
static LonStatusCode ProcessUplinkCodePacket(void);
static bool ValidateChecksum(uint8_t *pkt, size_t len);
static LonStatusCode ClearUplinkTransaction(void);
static LonStatusCode CheckUplinkCompleted(bool *completed);
static LonStatusCode ProcessUplinkMessage(void);
#else   // LINK_IS(MULTIPLE_USB_MIPS)
static LonStatusCode ProcessUplinkBytes(int iface_index, uint8_t *chunk,
        size_t chunk_size);
...
#if LINK_IS(MULTIPLE_USB_MIPS)  // Multiple interface states only needed for
                                // multiple USB MIP configuration
...
#if LINK_IS(USB_MIP)
static LonStatusCode ReadDownlinkBuffer(LonUsbQueueBuffer *dst);
static LonStatusCode FlushDownlinkQueue(MessagePriorityLevel priority);
static LonStatusCode QueueDownlinkBuffer(LonUsbQueueBuffer *src);
static size_t GetDownlinkBufferCount(void);
static bool IsDownlinkQueueEmpty(void);
#else   // LINK_IS(MULTIPLE_USB_MIPS)
...
#if LINK_IS(USB_MIP)
static LonStatusCode ReadUplinkBuffer(LonUsbQueueBuffer *dst);
static LonStatusCode QueueUplinkBuffer(void);
static size_t GetUplinkBufferCount(void);
static bool IsUplinkQueueEmpty(void);
#else   // LINK_IS(MULTIPLE_USB_MIPS)
...
#if LINK_IS(USB_MIP)
static void PrintUplinkCodePacket(char *prefix);
#else   // LINK_IS(MULTIPLE_USB_MIPS)
...
#if LINK_IS(USB_MIP)
static void IdentifyLonNi(void);
#else   // LINK_IS(MULTIPLE_USB_MIPS)
 ...
 *   		   and close calls for the interface (multiple USB MIPS only)
 *   lon_usb_iface_type: LON USB interface type (MIP/U50 or MIP/U61)
 *   line_discipline: USB line discipline number, set to LDISCS_50 for
 *			   the MIP/U50 (U10 FT Rev C and U60 FT), and LDISCS_61
 * for the MIP/U61 (U10 FT Rev A and B, U20 PL, U60 TP-1250, and U70 PL)
 * Returns:
 *   LonStatusNoError on success; LonStatusCode error code if unsuccessful
 */
LonStatusCode OpenLonUsbLink(
#if OS_IS(LINUX)
        char *lon_dev_name, char *usb_dev_name,
#endif  // OS_IS(LINUX)
#if LINK_IS(MULTIPLE_USB_MIPS)
        int *iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        LonUsbIfaceType lon_usb_iface_type, int usb_line_discipline)
{
    LonStatusCode status = LonStatusNoError;
    if (lon_usb_iface_type < 0 || lon_usb_iface_type >= MAX_IFACE_TYPE
#if OS_IS(LINUX)
            || !lon_dev_name || !usb_dev_name
#endif  // OS_IS(LINUX)
#if LINK_IS(MULTIPLE_USB_MIPS)
            || !iface_index
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        ...
        if (!LON_SUCCESS(status = InitIfaceStates())) {
            ...
            *iface_index = -1;
            ...
            OsalPrintLog(ERROR_LOG, status,
                    "OpenLonUsbLink: Failed to inititialize LON USB link "
                    "interface states");
            return status;
        }
    ...
    *iface_index = AssignIfaceState();
    if ((*iface_index < 0) || (*iface_index >= MAX_IFACE_STATES)) {
        *iface_index = -1;
        ...
        OsalPrintLog(ERROR_LOG, status,
                "OpenLonUsbLink: No available LON USB interface states");
        return status;
    }
    ...
    if (state == NULL) {
        ...
        *iface_index = -1;
        ...
        OsalPrintLog(ERROR_LOG, status,
                "OpenLonUsbLink: Failed to get LON USB interface state");
        return status;
    }
#endif                     // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
    ...
    OsalPrintLog(INFO_LOG, status,
            "OpenLonUsbLink: Starting LON USB link initialization in "
            "layer 2 mode");
#if OS_IS(LINUX)
    ...
    int usb_fd;
#endif  // OS_IS(LINUX)
    // Store LON USB interface type (MIP/U50 vs MIP/U61) in state
    state->lon_usb_iface_type = lon_usb_iface_type;
    // Initialize USB link and get the USB device file descriptor
    if (!LON_SUCCESS(status = HalOpenUsb(usb_line_discipline
#if OS_IS(LINUX)
                             ,
                             usb_dev_name, &usb_fd
#endif
...
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
        status = LonStatusOpenFailed;
        OsalPrintLog(ERROR_LOG, status, "OpenLonUsbLink: Failed to open USB interface");
        return status;
    }
#if OS_IS(LINUX)
    state->usb_fd = usb_fd;
    ...
    if (!LON_SUCCESS(status = StartLonUsbLink(LON_IFACE_MODE_LAYER2))) {
        ResetUplinkState();
#else   // LINK_IS(MULTIPLE_USB_MIPS)
    if (!LON_SUCCESS(status = StartLonUsbLink(*iface_index, LON_IFACE_MODE_LAYER2))) {
        HalCloseUsb(state->usb_fd);
        ResetUplinkState(*iface_index);
        *iface_index = -1;
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
        OsalPrintLog(ERROR_LOG, status, "OpenLonUsbLink: Failed to start LON USB link");
        return status;
    }
 ...
 */
static LonStatusCode StartLonUsbLink(
#if LINK_IS(MULTIPLE_USB_MIPS)
        int iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        LonUsbInterfaceMode configured_iface_mode)
{
    ...
    OsalPrintLog(INFO_LOG, status, "StartLonUsbLink: Layer %d mode requested",
            configured_iface_mode == LON_IFACE_MODE_LAYER5 ? 5 : 2);
...
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
        OsalPrintLog(ERROR_LOG, status,
                "StartLonUsbLink: Failed to flush downlink queue");
        return status;
    }
#if LINK_IS(USB_MIP)
...
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
        OsalPrintLog(ERROR_LOG, status,
                "StartLonUsbLink: Failed to reset downlink state");
        return status;
    }
#if LINK_IS(USB_MIP)
    if (!LON_SUCCESS(status = ResetUplinkState())) {
#else
    if (!LON_SUCCESS(status = ResetUplinkState(iface_index))) {
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
        OsalPrintLog(ERROR_LOG, status, "StartLonUsbLink: Failed to reset uplink state");
        return status;
    }
 ...
 */
#if LINK_IS(USB_MIP)
static LonStatusCode SendResync(void)
{
    LonUsbLinkState *state = &iface_state;
    if (state->shutdown) {
        return LonStatusInvalidInterfaceId;
...
static LonStatusCode SendResync(int iface_index)
{
    ...
    if (state->lon_usb_iface_type == LON_USB_INTERFACE_U50) {
        ...
        OsalPrintLog(INFO_LOG, status,
                "SendResync: Send resync command to LON interface");
#if LINK_IS(USB_MIP)
        if (!LON_SUCCESS(status = WriteDownlinkCodePacket(NI_RESYNC_FRAME_CMD, 0))) {
#else   // !LINK_IS(USB_MIP)
        if (!LON_SUCCESS(status = WriteDownlinkCodePacket(iface_index,
                                 NI_RESYNC_FRAME_CMD, 0))) {
#endif  // !LINK_IS(USB_MIP)
            OsalPrintLog(ERROR_LOG, status,
                    "SendResync: Failed to send resync command to LON "
                    "interface");
            return status;
        }
...
static LonStatusCode RequestUsbNiUid(int iface_index)
{
    ...
    if (state->have_uid) {
        ...
        OsalPrintLog(INFO_LOG, status,
                "RequestUsbNiUid: Already have LON interface unique ID, no "
                "need to request");
        return status;
        ...
        OsalPrintLog(ERROR_LOG, LonStatusTimeout,
                "RequestUsbNiUid: Failed to read LON interface unique ID "
                "after %d attempts",
                MAX_UID_RETRIES);
        return status;
    } else {
        ...
        OsalPrintLog(INFO_LOG, status,
                "RequestUsbNiUid: Request LON interface unique ID (%d retries "
                "left), %s state, %s state",
                state->uid_retries, downlink_state_names[state->downlink_state],
                uplink_state_names[state->uplink_state]);
#if LINK_IS(USB_MIP)
        if (!LON_SUCCESS(status = WriteLonUsbMsg(&MsgUniqueId))) {
#else   // !LINK_IS(USB_MIP)
        if (!LON_SUCCESS(status = WriteLonUsbMsg(iface_index, &MsgUniqueId))) {
#endif  // !LINK_IS(USB_MIP)
            return status;
        }
    }
 ...
 *   LonStatusNoError on success; LonStatusCode error code if unsuccessful
 * Notes:
 *   The iface_index paramater is not used with a single USB link configuration.
 */
LonStatusCode ReadUsbNiUid(
#if LINK_IS(MULTIPLE_USB_MIPS)
        int iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        IzotUniqueId *uid_buffer)
{
#if LINK_IS(USB_MIP)
    LonUsbLinkState *state = &iface_state;
    if (state->shutdown) {
...
static LonStatusCode RequestUsbNiStatus(int iface_index)
{
    ...
    if (state->have_status) {
        ...
        OsalPrintLog(INFO_LOG, status,
                "RequestUsbNiStatus: Already have LON interface status, no "
                "need to request");
        return status;
        ...
        OsalPrintLog(ERROR_LOG, LonStatusTimeout,
                "RequestUsbNiStatus: Failed to read LON interface status "
                "after %d attempts",
                MAX_STATUS_RETRIES);
        return status;
    } else {
        ...
        OsalPrintLog(INFO_LOG, status,
                "RequestUsbNiStatus: Request LON interface status (%d retries "
                "left), %s state, %s state",
                state->status_retries, downlink_state_names[state->downlink_state],
                uplink_state_names[state->uplink_state]);
#if LINK_IS(USB_MIP)
 ...
 *   LonStatusNoError on success; LonStatusCode error code if unsuccessful
 */
static LonStatusCode SetNiLayerMode(
#if LINK_IS(MULTIPLE_USB_MIPS)
        int iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        LonUsbInterfaceMode iface_mode)
{
#if LINK_IS(USB_MIP)
    LonUsbLinkState *state = &iface_state;
    if (state->shutdown) {
    ...
    // Mark the operating layer mode as unknown until confirmation is received
    // from the interface
...
#else   // LINK_IS(MULTIPLE_USB_MIPS)
    if (iface_mode == LON_IFACE_MODE_LAYER5) {
        status = WriteLonUsbMsg(iface_index, &MsgModeL5);
    } else {
        status = WriteLonUsbMsg(iface_index, &MsgModeL2);
    }
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
    return status;
}
...
/*
 * Sends pending downlink requests from the downlink queues to the LON interfaces. 
 * Parameters: 
 *   None 
 * Returns:    
 *   None 
 * Notes: 
 *   The LCS_Service() event loop functions call this function periodically to
 * process pending downlink messages and retry attempts for failed messages.
 */
void LonUsbDownlinkSend(void)
{
    LonStatusCode status = LonStatusNoError;
#if LINK_IS(USB_MIP)
...
#else   // LINK_IS(MULTIPLE_USB_MIPS)
    int ni_index;
    // Send the next LPDU from all downlink queues to the associated LON network
    // interfaces
    for (ni_index = 0; (LON_SUCCESS(status)) && (ni_index < MAX_IFACE_STATES);
            ni_index++) {
    ...
    if (!LON_SUCCESS(status)) {
        OsalPrintLog(ERROR_LOG, status,
                "LonUsbDownlinkSend: Failed to process downlink requests");
    }
}

/*
 * Processes retry and downlink requests for a specified LON USB network
 * interface. 
 * Parameters: 
 *   iface_index: LON interface index returned by OpenLonUsbLink()
 *                (multiple USB MIPS only)
    ...
    if (OsalGetTickCount() - last_report_time > 10000) {
        ...
        if (!IsDownlinkQueueEmpty() || !IsUplinkQueueEmpty()) {
            OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
                    "ProcessDownlinkRequests: Uptime %lu sec, %s state, %s "
                    "state, downlink queue count %zu, uplink queue count %zu",
                    (last_report_time - state->start_time) / 1000,
                    downlink_state_names[state->downlink_state],
                    uplink_state_names[state->uplink_state], GetDownlinkBufferCount(),
...
static LonStatusCode ProcessDownlinkRequests(int iface_index)
{
    ...
    if (state->downlink_state != last_downlink_state) {
        OsalPrintLog(PACKET_TRACE_LOG, status,
                ...
                "state changed from %s to %s, uplink state %s",
    ...
    case DOWNLINK_START:
        ...
        OsalPrintLog(INFO_LOG, status,
                "ProcessDownlinkRequests: Processing downlink restart");
#if LINK_IS(USB_MIP)
        ...
        state->wait_for_reset =
                true;  // Start by waiting for a reset to ensure the LON interface is in
                       // a known state before sending commands
    ...
    case DOWNLINK_WAIT_STARTUP_RESET:
        // Waiting for the network interface to reset on startup and after a
        // downlink restart
        if (state->have_reset ||
                (OsalGetTickCount() - state->reset_wait_timer > NI_RESET_WAIT_TIME)) {
            ...
            state->wait_for_null = true;  // Wait for null code packet to confirm the
                                          // interface is ready after reset
            state->null_wait_timer = OsalGetTickCount();
            state->downlink_state = DOWNLINK_WAIT_STARTUP_RESYNC_ACK;
        }
#if LINK_IS(USB_MIP)
    ...
    case DOWNLINK_WAIT_STARTUP_RESYNC_ACK:
        // Waiting for the network interface null code packet response that follows
        // a resynchronization request on startup
        if (state->have_null) {
            state->have_null = false;      // Clear null flag after handling null response
            state->wait_for_reset = true;  // Wait for reset to ensure the interface is
    ...
    case DOWNLINK_WAIT_RESYNC_RESET:
        // Waiting for the network interface to reset after a resynchronization
        // request on startup
        if (state->have_reset ||
                (OsalGetTickCount() - state->reset_wait_timer > NI_RESET_WAIT_TIME)) {
...
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
            OsalPrintLog(ERROR_LOG, status,
                    "ProcessDownlinkRequests: Failed to set LON interface mode "
                    "to layer 5");
            return status;
        }
        state->wait_for_null = true;  // Wait for null code packet to confirm the
                                      // interface is ready after layer mode change
        state->null_wait_timer = OsalGetTickCount();
    ...
    case DOWNLINK_NI_UNIQUE_ID_REQUEST:
        ...
        if (state->current_iface_mode != LON_IFACE_MODE_LAYER5) {
            OsalPrintLog(INFO_LOG, status,
                    "ProcessDownlinkRequests: Request layer 5 mode for unique "
                    "ID request, current mode %s",
                    state->current_iface_mode == LON_IFACE_MODE_LAYER2
                            ? "layer 2"
                            : (state->current_iface_mode == LON_IFACE_MODE_LAYER5
                                              ? "layer 5"
                                              : "unknown"));
...
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
            OsalPrintLog(ERROR_LOG, status,
                    "ProcessDownlinkRequests: Failed to request LON interface "
                    "unique ID after resync");
            return status;
        }
        state->uid_wait_timer = OsalGetTickCount();
        state->downlink_state = DOWNLINK_WAIT_NI_UNIQUE_ID;
    ...
    case DOWNLINK_WAIT_NI_UNIQUE_ID:
        ...
        if (state->have_uid) {
            ...
            if (state->wait_for_uid) {
                ...
                OsalPrintLog(ERROR_LOG, status,
                        "ProcessDownlinkRequests: Cannot read LON "
                        "interface unique ID");
                state->downlink_state = DOWNLINK_MIP_STATUS_REQUEST;
            }
        }
    ...
    case DOWNLINK_MIP_STATUS_REQUEST:
        ...
        if (state->current_iface_mode != LON_IFACE_MODE_LAYER2) {
            OsalPrintLog(ERROR_LOG, status,
                    "ProcessDownlinkRequests: LON interface not in expected "
                    "layer 2 mode after layer 2 mode set, current mode %s",
                    state->current_iface_mode == LON_IFACE_MODE_LAYER2
                            ? "layer 2"
                            : (state->current_iface_mode == LON_IFACE_MODE_LAYER5
                                              ? "layer 5"
    ...
    case DOWNLINK_IDLE_START:
        ...
        if (!LON_SUCCESS(status)) {
            OsalPrintLog(ERROR_LOG, status,
                    "ProcessDownlinkRequests: Cannot write downlink null "
                    "code packet");
            return status;
        }
        ...
        OsalPrintLog(INFO_LOG, status,
                "ProcessDownlinkRequests: LON interface initialization "
                "completed in %" PRIu32 " ms, entering idle state",
                state->lon_stats.startup_time);
        break;
    case DOWNLINK_IDLE:
#if LINK_IS(USB_MIP)
        status = ProcessNextDownlinkMessage();
    ...
    case DOWNLINK_WAIT_MSG_ACK:
        ...
        if (++state->uplink_ack_timeouts >= 2) {
            if (state->uplink_ack_timeout_phase < 5) {
                ...
                // Overwrite the downlink message at
                // state->downlink_buffer.usb_ni_message
                memcpy(&state->downlink_buffer.usb_ni_data_frame, &MsgReportStatus,
                        sizeof(MsgReportStatus));
                ...
                OsalPrintLog(PACKET_TRACE_LOG, LonStatusTimeout,
                        "LON interface ACK timeout phase %d, sequence #%02X, "
                        "requesting status",
                        state->uplink_ack_timeout_phase, state->downlink_seq_number);
                OsalPrintLog(PACKET_TRACE_LOG, LonStatusTimeout,
                        "LON interface checksum errors: %d  code packet "
                        "failures: %d",
                        state->lon_stats.uplink.rx_checksum_errors,
                        state->lon_stats.uplink.rx_code_packet_failures);
                OsalPrintLog(PACKET_TRACE_LOG, LonStatusTimeout,
                        "LON interface packets received: %d, bytes "
                        "received: %d",
                        state->lon_stats.uplink.packets_received,
                        state->lon_stats.uplink.bytes_received);
                OsalPrintLog(PACKET_TRACE_LOG, LonStatusTimeout,
                        "LON interface TX state %s, RX state %s",
                        downlink_state_names[starting_downlink_state],
                        uplink_state_names[state->uplink_state]);
            } else {  // Do this once
                OsalTickCount current_time = OsalGetTickCount();
                // If we have a MAC address and it's been more than 60 seconds since
                // startup or the last timeout then panic and reset the interface
                if (!state->wait_for_uid &&
                        (current_time - state->last_timeout) > STARTUP_DELAY) {
                    OsalPrintLog(ERROR_LOG, LonStatusTimeout,
                            "LON interface ACK timeout limit #%d, restart now",
                            state->uplink_ack_timeout_phase);
                    state->last_timeout =
                            current_time;  // Don't keep doing this too rapidly
                } else {                   // Try again
                    state->uplink_ack_timeout_phase = 0;
                }
            }
            ...
            OsalPrintLog(PACKET_TRACE_LOG, LonStatusTimeout,
                    "LON interface ACK timeout (first case)");
        }
#if LINK_IS(USB_MIP)
        status = ProcessNextDownlinkMessage();
#else   // !LINK_IS(USB_MIP)
...
/*
 * Processes the next available downlink message for a specified LON USB network
 * interface. 
 * Parameters: 
 *   iface_index: LON interface index returned by OpenLonUsbLink()
 *             (multiple USB MIPS only)
 ...
 *   LonStatusNoError on success; LonStatusCode error code if unsuccessful
 * Notes: 
 *   This function writes the next available downlink message from the downlink
 *   queue and to the USB interface. It is called from multiple states in
 *   ProcessDownlinkRequests().
 */
#if LINK_IS(USB_MIP)
static LonStatusCode ProcessNextDownlinkMessage(void)
...
static LonStatusCode ProcessNextDownlinkMessage(int iface_index)
{
    ...
    if (state->downlink_ack_seq_number >= 0) {
        ...
        OsalPrintLog(PACKET_TRACE_LOG, status,
                "ProcessNextDownlinkMessage: Sending null code packet to send "
                "ACK, ack sequence #%02X",
        ...
        if (!LON_SUCCESS(status)) {
            OsalPrintLog(ERROR_LOG, status,
                    "ProcessNextDownlinkMessage: Cannot write downlink "
                    "null code packet");
            return status;
        }
#if LINK_IS(USB_MIP)
        ...
        if (!LON_SUCCESS(status)) {
            OsalPrintLog(ERROR_LOG, status,
                    "ProcessNextDownlinkMessage: Cannot increment "
                    "sequence number");
            return status;
        }
    }
    ...
    if (!LON_SUCCESS(status)) {
        OsalPrintLog(ERROR_LOG, status,
                "ProcessNextDownlinkMessage: Cannot read downlink buffer for "
                "LON interface");
        return status;
    }
    ...
    LonNiCommand cmd =
            state->downlink_buffer.usb_ni_data_frame.ni_command;  // Extracted early
    OsalPrintLog(PACKET_TRACE_LOG, status,
            "ProcessNextDownlinkMessage: Processing downlink message, "
            "command 0x%02X, short PDU length %d, current downlink state %s",
            cmd, short_pdu_length, downlink_state_names[state->downlink_state]);
    if ((state->lon_usb_iface_type == LON_USB_INTERFACE_U50) && (short_pdu_length == 1) &&
    ...
    if (!LON_SUCCESS(status)) {
        OsalPrintLog(ERROR_LOG, status,
                "ProcessNextDownlinkMessage: Cannot write downlink message");
        return status;
    }
    return status;
}
 ...
 *   in layer 5 mode.
 */
LonStatusCode WriteLonUsbMsg(
#if LINK_IS(MULTIPLE_USB_MIPS)
        int iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        const LonDataFrame *input_frame)
{
#if LINK_IS(USB_MIP)
    LonUsbLinkState *state = &iface_state;
    if (state->shutdown) {
    ...
    if (GetDownlinkBufferCount(iface_index) < MAX_LON_DOWNLINK_BUFFERS) {
        ...
        status = CreateFrameHeader(&state->downlink_buffer_staging.frame_header,
                MSG_FRAME_CMD, 0, 0,
                true);  // TODO: Determine if parameter should be
                        // 1 for 1 message following
        OsalPrintLog(DETAIL_TRACE_LOG, LonStatusNoError,
                "WriteLonUsbMsg: Created frame header with frame sync 0x%02X, "
                "frame code 0x%02X, parameter 0x%02X, checksum 0x%02X",
        ...
        if (LON_SUCCESS(status)) {
            if (input_frame->short_pdu_length == EXT_LENGTH) {
                // This is an extended message (length > 254)--cast source to extended
                // frame
                const LonExtDataFrame *input_ext_frame = (LonExtDataFrame *)input_frame;
                // Create extended USB network interface destination frame
                LonNiExtendedFrame *usb_ni_ext_frame =
                        (LonNiExtendedFrame *)&state->downlink_buffer_staging
                                .usb_ni_data_frame;
                // Get extended length from source frame and convert to host byte order
                size_t ext_length = ntoh16(input_ext_frame->ext_length_be);
                if (ext_length <= MAX_LON_MSG_EX_LEN) {
                    ...
                    usb_ni_ext_frame->ext_length_be = hton16(ext_length + 1);
                    // Copy network interface command
                    usb_ni_ext_frame->ni_command = input_ext_frame->ni_command;
                    // Copy source to destination extended PDU, excluding length and flag
                    // bytes copied above
                    memcpy(&usb_ni_ext_frame->ext_pdu, &input_ext_frame->ext_pdu,
                            ext_length - 3);
                    // Update sizes
                    // TODO: Check this calculation; the ext_length includes the
                    // ni_command byte and the ext_pdu length, but the ext_pdu in the
                    // destination frame does not include the ni_command byte
                    state->downlink_buffer_staging.buf_size =
                            sizeof(LonFrameHeader) + sizeof(LonNiExtendedFrame) -
                            sizeof(usb_ni_ext_frame->ext_pdu) + ext_length;
                    state->downlink_buffer_staging.data_frame_size =
                            state->downlink_buffer_staging.buf_size -
                            sizeof(LonFrameHeader);
                    status = LonStatusNoError;
                } else {
                ...
                LonNiFrame *usb_ni_data_frame =
                        &state->downlink_buffer_staging.usb_ni_data_frame;
                ...
                if (state->lon_usb_iface_type == LON_USB_INTERFACE_U50 &&
                        (lonNiCommand == LonNiLayerModeCmd &&
                                adjusted_short_pdu_length)) {
                    // For MIP/U50 layer mode command, eliminate the network interface
                    // command parameter byte from the PDU and adjust lengths
                    adjusted_short_pdu_length = 0;
                    usb_ni_data_frame->short_pdu_length = adjusted_short_pdu_length + 1;
                    usb_ni_data_frame->ni_command = (input_frame->pdu[0] == 0)
                                                            ? LonNiSetL5ModeCmd
                                                            : LonNiSetL2ModeCmd;
                    // Set the interface mode used for this link after NI reset
                    state->configured_iface_mode = (input_frame->pdu[0] == 0)
                                                           ? LON_IFACE_MODE_LAYER5
                                                           : LON_IFACE_MODE_LAYER2;
                } else {
                    // Increment length to include network interface command byte (TODO:
                    // Is this checksum instead?)
                    usb_ni_data_frame->short_pdu_length = adjusted_short_pdu_length + 1;
                    // Copy network interface command
                ...
                if (adjusted_short_pdu_length) {
                    ...
                    memcpy(usb_ni_data_frame->pdu, input_frame->pdu,
                            adjusted_short_pdu_length);
                }
                // Update sizes
                size_t unused_pdu_bytes =
                        sizeof(usb_ni_data_frame->pdu) - adjusted_short_pdu_length;
                state->downlink_buffer_staging.buf_size =
                        sizeof(LonUsbQueueBuffer) - unused_pdu_bytes;
                state->downlink_buffer_staging.data_frame_size =
                        sizeof(LonNiFrame) - unused_pdu_bytes;
                status = LonStatusNoError;
            }
        }
        state->downlink_buffer_staging.priority =
                (input_frame->ni_command == (LonNiCommandMask | LonNiPriorityTxnQueue))
                        ? HIGH_PRIORITY
                        : NORMAL_PRIORITY;
        if (LON_SUCCESS(status)) {
            OsalPrintLog(INFO_LOG, LonStatusNoError,
                    ...
                    state->downlink_buffer_staging.buf_size,
                    state->downlink_buffer_staging.usb_ni_data_frame.ni_command);
#if LINK_IS(USB_MIP)
            ...
            if (!LON_SUCCESS(status)) {
                OsalPrintLog(ERROR_LOG, status,
                        "WriteLonUsbMsg: Failed to queue downlink buffer");
            }
        }
    }
...
{
    OsalPrintLog(DETAIL_TRACE_LOG, LonStatusNoError,
            "CreateFrameHeader: Creating frame header with frame command "
            "0x%02X, parameter 0x%02X, sequence %d, ack %s",
    ...
    frame_header->checksum =
            -ComputeChecksum((uint8_t *)frame_header, sizeof(LonFrameHeader) - 1);
    OsalPrintLog(DETAIL_TRACE_LOG, LonStatusNoError,
            "CreateFrameHeader: Created frame header with frame sync "
            "0x%02X, frame code 0x%02X, parameter 0x%02X, checksum 0x%02X",
            frame_header->frame_sync, frame_header->frame_code, frame_header->parameter,
            frame_header->checksum);
    return LonStatusNoError;
}
        ...
        uint8_t *pkt, size_t len)
{
    ...
    if (!is_valid) {
        OsalPrintLog(ERROR_LOG, LonStatusChecksumError,
                "ValidateChecksum: Invalid checksum computed for %zu-byte "
                "packet",
                len);
...
static LonStatusCode WriteDownlinkMessage(int iface_index)
{
...
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
    LonStatusCode status = LonStatusNoError;
    uint8_t *dest_exp_ni_data_frame_ptr, *src_data_frame, *src_data_frame_ptr,
            message_byte;
    ...
    while (remaining--) {
        ...
        if (expanded_length >= sizeof(state->exp_downlink_ni_data_frame) - 2) {
            status = LonStatusInvalidBufferLength;
            OsalPrintLog(ERROR_LOG, status,
                    "WriteDownlinkMessage: Expanded downlink message exceeds "
                    "buffer size");
            return status;
        }
    }
    ...
    OsalPrintLog(DETAIL_TRACE_LOG, status,
            ...
            "%zu bytes before expansion, %zu bytes after expansion, seq %d",
            state->downlink_buffer.usb_ni_data_frame.ni_command, unexpanded_length,
            expanded_length, state->downlink_seq_number);
    OsalPrintMessage(DETAIL_TRACE_LOG,
            "WriteDownlinkMessage (not expanded): ", src_data_frame, unexpanded_length);
    OsalPrintMessage(DETAIL_TRACE_LOG,
            "WriteDownlinkMessage (expanded):     ", state->exp_downlink_ni_data_frame,
            expanded_length);
...
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
            OsalPrintLog(ERROR_LOG, status,
                    "WriteDownlinkMessage: Failed to start ACK timer for "
                    "downlink message");
            return status;
        }
    }
    ...
    status = HalWriteUsb(state->usb_fd, state->exp_downlink_ni_data_frame,
            expanded_length, &bytes_written);
    if (!LON_SUCCESS(status) || bytes_written != expanded_length) {
        OsalPrintLog(INFO_LOG, status,
                "WriteDownlinkMessage: Wrote %zu of %d bytes to LON USB "
                "interface",
                bytes_written, expanded_length);
 ...
 *   LonStatusNoError on success; LonStatusCode error code if unsuccessful
 */
static LonStatusCode StartAckTimer(
#if LINK_IS(MULTIPLE_USB_MIPS)
        int iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        DownlinkState next_state)
{
#if LINK_IS(USB_MIP)
    LonUsbLinkState *state = &iface_state;
    if (state->shutdown) {
 ...
 *   LonStatusNoError on success; LonStatusCode error code if unsuccessful
 */
static LonStatusCode WriteDownlinkCodePacket(
#if LINK_IS(MULTIPLE_USB_MIPS)
        int iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        LonUsbFrameCommand frame_cmd, uint8_t parameter)
{
    ...
    if ((state->downlink_ack_seq_number >= 0)) {
        ...
        IZOT_SET_ATTRIBUTE(frame_header, IZOT_U50_FRAME_CODE_SEQ, sequence_num);
    } else if ((frame_cmd == MSG_FRAME_CMD) ||
               (state->downlink_ack_seq_number == ACK_WITH_NEXT_SEQ_NUM)) {
    ...
    frame_header.checksum =
            -ComputeChecksum((uint8_t *)&frame_header, sizeof(frame_header) - 1);
    // Send the code packet
    size_t bytes_written = 0;
    status = HalWriteUsb(state->usb_fd, (uint8_t *)&frame_header, sizeof(frame_header),
            &bytes_written);
    if (!LON_SUCCESS(status)) {
        OsalPrintLog(ERROR_LOG, status,
                "WriteDownlinkCodePacket: Failed to write downlink code "
                "packet with command 0x%02X, parameter 0x%02X, sequence %02X",
                frame_cmd, parameter, sequence_num);
        return status;
    }
    if (bytes_written != sizeof(frame_header)) {
        OsalPrintLog(ERROR_LOG, LonStatusLniWriteFailure,
                "WriteDownlinkCodePacket: Failed to write downlink code "
                "packet with command 0x%02X, parameter 0x%02X, sequence %02X",
                frame_cmd, parameter, sequence_num);
        return LonStatusLniWriteFailure;
    }
    // Reset sequence number to 0 for the next message if this is a sync frame
    // code
    ...
    OsalPrintLog(DETAIL_TRACE_LOG, LonStatusNoError,
            "WriteDownlinkCodePacket: Sent downlink code packet with "
            "command 0x%02X, parameter 0x%02X, sequence %02X",
            frame_cmd, parameter, sequence_num);
    return status;
}
...
static LonStatusCode WriteDownlinkLocalNiCmd(int iface_index)
{
    ...
    DownlinkState next_downlink_state =
            state->uplink_expected_rsp ? DOWNLINK_WAIT_CP_RESPONSE : DOWNLINK_WAIT_CP_ACK;
    ...
    OsalPrintLog(INFO_LOG, LonStatusNoError,
            "Send code packet with local NI command 0x%02X with 0x%02X "
            "expected response",
    ...
    if (!LON_SUCCESS(status)) {
        OsalPrintLog(ERROR_LOG, status,
                "Failed to write downlink local NI command 0x%02X", cmd);
        return status;
    }
    if (state->downlink_state >= DOWNLINK_IDLE_START) {
...
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
            OsalPrintLog(ERROR_LOG, status,
                    "Failed to start ACK timer for local NI command 0x%02X", cmd);
            return status;
        }
    }
    return status;
 ...
 *   1. The host sends a SHORT_NI_CMD_FRAME_CMD code packet that requires a
 * response.
 ...
 *      with an ACK to a previous command because the host has not yet
 * acknowledged a previously pending uplink short command.
 *   3. The host sends another SHORT_NI_CMD_FRAME_CMD code packet overwriting
 * the command sent in number 1. To prevent losing the first command, a downlink
 * short command is not acknowledged if there is another downlink short command
 * pending.  Instead, the host waits for the expected response to the command
 * before sending an ACK. Called by WriteDownlinkLocalNiCmd().
 */
static LonNiCommand GetExpectedResponse(uint8_t cmd)
{
 ...
 *   iface_index: interface index returned by OpenLonUsbLink()
 *             (multiple USB MIPS only)
 *   out_msg: pointer to LonDataFrame or LdvExtendedMessage structure
 * Returns:
 ...
 */
LonStatusCode ReadLonUsbMsg(
#if LINK_IS(MULTIPLE_USB_MIPS)
        int iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        LonDataFrame *out_msg)
{
#if LINK_IS(USB_MIP)
    LonUsbLinkState *state = &iface_state;
    if (state->shutdown) {
    ...
    int fd = state->usb_fd;
    // Check available ring capacity under queue_lock to avoid over-pulling from
    // USB
    size_t rb_avail = 0;
    // Lock ring to safely check available capacity; another thread may feed
    // concurrently Protect queue head access; other threads may enqueue/dequeue
    // concurrently
    OsalLockQueue(&state->queue_lock);
    rb_avail = RingBufferAvail(&state->lon_usb_uplink_ring_buffer);
    OsalUnlockQueue(&state->queue_lock);  // Keep critical section minimal
    if (fd >= 0 && rb_avail > MAX_BYTES_PER_USB_READ) {
        ...
        if (LON_SUCCESS(status) && bytes_read > 0) {
            ...
            size_t written = RingBufferWrite(&state->lon_usb_uplink_ring_buffer,
                    temp_read_buf, (size_t)bytes_read);
            ...
            if (written < (size_t)bytes_read) {
                ...
                OsalUnlockQueue(&state->queue_lock);  // Release before logging
                OsalPrintLog(ERROR_LOG, LonStatusOverflow,
                        "ReadLonUsbMsg: RX ring overflow, dropped %zu of "
                        "%zd bytes",
                        dropped, bytes_read);
            } else {
                OsalUnlockQueue(&state->queue_lock);  // Leave lock before returning
            }
    ...
    OsalUnlockQueue(
            &state->queue_lock);  // Free outside of lock to keep critical section small
#if LINK_IS(USB_MIP)
    ...
    if (ring_count > 0 && GetUplinkBufferCount(iface_index) < MAX_LON_UPLINK_BUFFERS) {
        ...
        for (;;) {
            ...
            if (cur_count == 0) {
                OsalUnlockQueue(&state->queue_lock);
                break;
            }
            size_t chunk_size = RingBufferRead(&state->lon_usb_uplink_ring_buffer,
                    parse_chunk, sizeof(parse_chunk));
            OsalUnlockQueue(&state->queue_lock);
            if (chunk_size == 0) {
...
#if LINK_IS(USB_MIP)
            if (GetUplinkBufferCount() > 0) {
                break;
            }
#else   // LINK_IS(MULTIPLE_USB_MIPS)
            if (GetUplinkBufferCount(iface_index) > 0) {
                break;
            }
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
            // Guard against monopolizing CPU if ring is very full; parse only a
            // window per call
            if (processed_in_window > MAX_BYTES_PER_USB_PARSE_WINDOW) {
                break;  // Arbitrary processing budget
            }
        }
    }
...
#if LINK_IS(USB_MIP)
    if ((!LON_SUCCESS(status = ReadUplinkBuffer(&buffer))) &&
            status != LonStatusNoBufferAvailable) {
        return status;
    }
#else   // LINK_IS(MULTIPLE_USB_MIPS)
    if ((!LON_SUCCESS(status = ReadUplinkBuffer(iface_index, &buffer))) &&
            status != LonStatusNoBufferAvailable) {
        return status;
    }
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
    if (status != LonStatusNoBufferAvailable) {
        ...
        // Field assignment converts from LonNiFrame to LonDataFrame; the PDU is
        // copied separately below
        out_msg->ni_command = buffer.usb_ni_data_frame.ni_command;
        // TODO: Verify extended message handling.  short_pdu_length includes the
        // command byte
        // TODO: Verify the -1 for PDU length; the length sent by the MIP includes
        // the command byte, but the command byte is not included in the PDU field
...
/*
 * Interrupt Service Routine to handle an uplink message interrupt from the LON
 * USB interface. Parameters: iface_index: interface index returned by
 * OpenLonUsbLink() (multiple USB MIPS only) usb_data: pointer to byte array of
 * data read from the USB interface usb_data_len: length of the usb_data buffer.
 * Returns:
 *   None.  Errors are logged internally.
 * Notes:
 *   Called by the USB driver when an uplink message is received from the
 *   LON USB interface.  Writes the received bytes into the uplink ring buffer
 *   for subsequent processing by ReadLonUsb().  Not used for polling USB
 *   implementations.
 */
#if USB_UPLINK_IS(INTERRUPT)
void LonUsbMsgIsr(
#if LINK_IS(MULTIPLE_USB_MIPS)
        int iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        uint8_t *usb_data, size_t usb_data_len)
{
    OsalInterruptStatus interrupt_status;
    if (!usb_data || usb_data_len == 0) {
        return;
    }
#if LINK_IS(USB_MIP)
    LonUsbLinkState *state = &iface_state;
    ...
    // concurrently Protect queue head access; other threads may enqueue/dequeue
    // concurrently
    OsalLockQueueFromIsr(&state->queue_lock, interrupt_status);
    rb_avail = RingBufferAvail(&state->lon_usb_uplink_ring_buffer);
    OsalUnlockQueueFromIsr(&state->queue_lock,
            interrupt_status);  // Keep critical section minimal
    if (rb_avail > 0) {
        // Lock ring while writing uplink bytes and updating stats
        OsalLockQueueFromIsr(&state->queue_lock, interrupt_status);
        written = RingBufferWrite(&state->lon_usb_uplink_ring_buffer, usb_data,
                usb_data_len);
        size_t occ = RingBufferSize(&state->lon_usb_uplink_ring_buffer);
        state->lon_stats.usb_rx.bytes_fed += written;
        if (occ > state->lon_stats.usb_rx.max_occupancy) {
        ...
        OsalUnlockQueueFromIsr(&state->queue_lock,
                interrupt_status);  // Unlock before returning
    }
    if (written < usb_data_len) {
        size_t dropped = usb_data_len - written;
        state->lon_stats.usb_rx.bytes_dropped += dropped;
        OsalPrintLog(ERROR_LOG, LonStatusOverflow,
                "LonUsbMsgIsr: RX ring buffer overflow, dropped %zu of "
                "%zu bytes",
                dropped, usb_data_len);
    }
}
#endif  // USB_UPLINK_IS(INTERRUPT)
 ...
 * 			   -- 1: for CpMsg (2)
 * 			   -- <command>: for CpNiCmdShort (6); and for LonNiResetDeviceCmd (0x50) (see smpDlNiCmdLocal)
 *   - Bytes 3: For MIP/U50 only: checksum byte (negated sum of all bytes modulo 256)
 *   - Bytes 4+: PDU data bytes (if any)
 *
 *   For MIP/U50:
 *   - Send CpNiCmdShort/LonNiSetL2ModeCmd (0xD1), CpNiCmdShort/LonNiStatusCmd
 *     (0xE0), and CpNiCmdShort/0 commands to set layer 2 mode
 ...
 *   - Respond to LonNiStatusCmd (0xE0), LonNiSetL5ModeCmd (0xD0), and
 *     LonNiSetL2ModeCmd (0xD1) commands with the same command
 ...
 */
static LonStatusCode ProcessUplinkBytes(
#if LINK_IS(MULTIPLE_USB_MIPS)
        int iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        uint8_t *chunk, size_t chunk_size)
{
    ...
    if (last_uplink_state != state->uplink_state) {
        OsalPrintLog(DETAIL_TRACE_LOG, LonStatusNoError,
                "ProcessUplinkBytes: Uplink state changed from %s to %s",
                uplink_state_names[last_uplink_state],
                uplink_state_names[state->uplink_state]);
    ...
    while (chunk_size && !state->shutdown && LON_SUCCESS(status)) {
        ...
        case UPLINK_IDLE:
            ...
            if (*chunk == FRAME_SYNC) {
                ...
                state->uplink_frame_error = FALSE;
                OsalPrintLog(DETAIL_TRACE_LOG, LonStatusNoError,
                        "ProcessUplinkBytes: Uplink frame sync received, "
                        "change "
                        "state to UPLINK_FRAME_CODE");
                ...
                OsalPrintLog(ERROR_LOG, LonStatusFrameError,
                        "ProcessUplinkBytes: First uplink frame error, %02X "
                        "received 0x7E expected",
                        *chunk);
        ...
        case UPLINK_FRAME_CODE:
            if (state->lon_usb_iface_type == LON_USB_INTERFACE_U50) {
                ...
                IZOT_SET_ATTRIBUTE(state->uplink_buffer.frame_header,
                        IZOT_U50_FRAME_CODE_ACK, (*chunk & 0x10) ? 1 : 0);
                IZOT_SET_ATTRIBUTE(state->uplink_buffer.frame_header,
                        IZOT_U50_FRAME_CODE_SEQ, (*chunk >> 5) & 0x07);
                IZOT_SET_ATTRIBUTE(state->uplink_buffer.frame_header,
                        IZOT_U50_FRAME_CODE_CMD, *chunk & 0x0F);
                state->uplink_state = UPLINK_FRAME_PARAMETER;
                OsalPrintLog(DETAIL_TRACE_LOG, LonStatusNoError,
                        "ProcessUplinkBytes: Uplink frame code received: cmd "
                        "0x%02X, seq "
                        "%d, ack %d; change state to UPLINK_FRAME_PARAMETER",
                        IZOT_GET_ATTRIBUTE(state->uplink_buffer.frame_header,
                                IZOT_U50_FRAME_CODE_CMD),
                        IZOT_GET_ATTRIBUTE(state->uplink_buffer.frame_header,
                                IZOT_U50_FRAME_CODE_SEQ),
                        IZOT_GET_ATTRIBUTE(state->uplink_buffer.frame_header,
                                IZOT_U50_FRAME_CODE_ACK));
                chunk++;
                chunk_size--;
            } else {
        ...
        case UPLINK_FRAME_PARAMETER:
            ...
            OsalPrintLog(DETAIL_TRACE_LOG, LonStatusNoError,
                    "ProcessUplinkBytes: Uplink frame parameter received: "
                    "0x%02X; change state to UPLINK_CODE_PACKET_CHECKSUM",
...
#else   // LINK_IS(MULTIPLE_USB_MIPS)
            if (!ValidateChecksum(iface_index,
                        (uint8_t *)&state->uplink_buffer.frame_header,
                        sizeof(state->uplink_buffer.frame_header))) {
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
                ...
                state->downlink_ack_seq_number = NO_ACK_REQUIRED;
                OsalPrintLog(ERROR_LOG, LonStatusFrameError,
                        "ProcessUplinkBytes: Uplink code packet checksum "
                        "error");
                state->uplink_state = UPLINK_IDLE;
            } else {
                ...
                if (IZOT_GET_ATTRIBUTE(state->uplink_buffer.frame_header,
                            IZOT_U50_FRAME_CODE_CMD) == MSG_FRAME_CMD) {
        ...
        case UPLINK_MESSAGE:
            while (chunk_size) {
                uint8_t msg_byte = ((uint8_t *)&state->uplink_buffer
                                .usb_ni_data_frame)[state->uplink_msg_index++] = *chunk++;
                chunk_size--;
            ...
            if (state->uplink_state != UPLINK_IDLE &&
                    (OsalGetTickCount() - state->uplink_msg_timer) >
                            state->usb_params.llp_timeout) {
                Increment32(state->lon_stats.uplink.rx_timeout_errors);
                OsalPrintLog(INFO_LOG, LonStatusFrameTimeout,
                        "ProcessUplinkBytes: Uplink frame timeout");
#if LINK_IS(USB_MIP)
                status = ResetUplinkState();
#else   // LINK_IS(MULTIPLE_USB_MIPS)
        ...
        case UPLINK_ESCAPED_DATA:
            ...
            if (*chunk != FRAME_SYNC) {
                ...
                state->downlink_ack_seq_number = NO_ACK_REQUIRED;
                OsalPrintLog(ERROR_LOG, LonStatusFrameError,
                        "ProcessUplinkBytes: Uplink frame error (data)");
                if (*chunk == 0) {
                    state->uplink_msg_index = 0;
                } else {
#if LINK_IS(USB_MIP)
        ...
        default:
            OsalPrintLog(ERROR_LOG, LonStatusInternalError,
                    "ProcessUplinkBytes: Invalid uplink state %d", state->uplink_state);
            break;
        }
    }
    return status;
...
/*
 * Processes an uplink code packet after the full packet has been received and validated.
 * Parameters: 
 *   iface_index: interface index returned by OpenLonUsbLink() (multiple USB MIPS only) 
 * Returns:
 *   LonStatusNoError on success; LonStatusCode error code if unsuccessful 
 * Notes:
 *   For MIP/U50, the code packet header includes a command, parameter, sequence number, and ACK
 *   flag; for MIP/U61, the code packet header includes only a command.
 */
#if LINK_IS(USB_MIP)
static LonStatusCode ProcessUplinkCodePacket(void)
{
...
static LonStatusCode ProcessUplinkCodePacket(int iface_index)
{
    ...
    LonUsbFrameCommand frame_cmd =
            IZOT_GET_ATTRIBUTE(*frame_header, IZOT_U50_FRAME_CODE_CMD);
    ...
    bool cp_fail_or_reject =
            ((frame_cmd == FAIL_FRAME_CMD) || (frame_cmd == MSG_REJECT_FRAME_CMD));
    if (frame_cmd) {
    ...
    if (ack_flag && !cp_fail_or_reject) {
        OsalPrintLog(DETAIL_TRACE_LOG, status,
                "ProcessUplinkCodePacket: ACK flag set for sequence %d", sequence_number);
        if (state->downlink_state != DOWNLINK_WAIT_CP_RESPONSE) {
            // Process ACK flag for good code packets that are not local
    ...
    switch (frame_cmd) {
    case MSG_FRAME_CMD:
        // Message frame header received; change state to receive message in
        // CheckUplinkCompleted()
        OsalPrintLog(PACKET_TRACE_LOG, status,
                "ProcessUplinkCodePacket: Received uplink message frame with "
                "sequence %d, ack %d, parameter 0x%02X",
                sequence_number, ack_flag, parameter);
        state->downlink_ack_seq_number = sequence_number;
        if (ack_flag) {
            OsalPrintLog(DETAIL_TRACE_LOG, status,
                    "ProcessUplinkCodePacket: ACK flag set for message "
                    "with sequence %d",
                    sequence_number);
        }
        state->uplink_state = UPLINK_MESSAGE;
    ...
    case SHORT_NI_CMD_FRAME_CMD:
        ...
        OsalPrintLog(PACKET_TRACE_LOG, status,
                "ProcessUplinkCodePacket: Received uplink short NI command "
                "with sequence %d, ack %d, NI command 0x%02X",
                sequence_number, ack_flag, parameter);
        state->downlink_ack_seq_number = sequence_number;
        if (ack_flag) {
            OsalPrintLog(PACKET_TRACE_LOG, status,
                    "ProcessUplinkCodePacket: ACK flag set for short NI "
                    "command "
                    "with sequence %d",
                    sequence_number);
        }
        if (!state->uplink_duplicate) {
            if (parameter == LonNiAckCmd || parameter == LonNiNackCmd) {
                ...
                // TODO: Is "state->downlink_ack_required = false;" required here, or is
                // it sufficient that these commands are not processed as part of an
                // uplink transaction in ClearUplinkTransaction()?
                break;
            }
...
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
                OsalPrintLog(ERROR_LOG, status,
                        "ProcessUplinkCodePacket: Failed to queue uplink "
                        "short NI "
                        "command 0x%02X",
                        parameter);
                return status;
    ...
    case MSG_REJECT_FRAME_CMD:
        ...
        if (state->downlink_state == DOWNLINK_WAIT_MSG_ACK ||
                state->downlink_state == DOWNLINK_WAIT_CP_ACK) {
            ...
            if (!state->downlink_reject_timer) {
                ...
                OsalPrintLog(INFO_LOG, status,
                        "ProcessUplinkCodePacket: Reject received from LON "
                        "interface downlink reject timer started");
            } else if (OsalGetTickCount() - state->downlink_reject_timer >
                       DOWNLINK_WAIT_TIME) {
                state->downlink_reject_timer = 0;
                // Downlink reject timer limit reached; TODO: Reset the network
                // interface
                status = LonStatusLniWriteFailure;
                OsalPrintLog(ERROR_LOG, status,
                        "ProcessUplinkCodePacket: Downlink reject timer "
                        "expired; "
                        "resetting LON interface");
#if LINK_IS(USB_MIP)
                WriteDownlinkCodePacket(SHORT_NI_CMD_FRAME_CMD, LonNiResetDeviceCmd);
#else   // LINK_IS(MULTIPLE_USB_MIPS)
                WriteDownlinkCodePacket(iface_index, SHORT_NI_CMD_FRAME_CMD,
                        LonNiResetDeviceCmd);
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
            }
            ...
            OsalPrintLog(PACKET_TRACE_LOG, status,
                    "ProcessUplinkCodePacket: Unknown CP or message reject "
                    "cause for LON interface in state %d.",
    ...
    case FAIL_FRAME_CMD:
        ...
        status = StopAckTimer();
#else   // LINK_IS(MULTIPLE_USB_MIPS)
        ClearUplinkTransaction(iface_index);  // Don't keep repeating a failed
                                              // message
        status = StopAckTimer(iface_index);
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
        ...
        OsalPrintLog(ERROR_LOG, status,
                "ProcessUplinkCodePacket: Received uplink failure frame with "
                "sequence %d, ack %d, parameter 0x%02X",
                sequence_number, ack_flag, parameter);
        break;
    case NULL_FRAME_CMD:
        OsalPrintLog(PACKET_TRACE_LOG, status,
                "ProcessUplinkCodePacket: Received uplink null frame with "
                "sequence %d, ack %d, parameter 0x%02X",
                sequence_number, ack_flag, parameter);
    ...
    default:
        ...
        status = LonStatusFrameError;
        OsalPrintLog(ERROR_LOG, status,
                "ProcessUplinkCodePacket: Received uplink frame with unknown "
                "command 0x%02X",
                frame_cmd);
        break;
    }
...
// TODO: Verify message processing in the original LON U60 driver after
// ProcessUplinkCodePacket()

/*
 * Clears any pending uplink transaction for a specified LON USB network
 * interface.
 * Parameters:
 *   iface_index: LON interface index returned by OpenLonUsbLink() (multiple USB MIPS only) 
 * Returns:
 *   LonStatusNoError on success; LonStatusCode error code if unsuccessful 
 * Notes:
 *   Called to complete or abort any pending uplink transaction before starting a new uplink
 *   transaction.
 */
#if LINK_IS(USB_MIP)
static LonStatusCode ClearUplinkTransaction(void)
...
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
    LonStatusCode status = LonStatusNoError;
    if (state->downlink_state == DOWNLINK_WAIT_CP_ACK ||
            state->downlink_state == DOWNLINK_WAIT_CP_RESPONSE) {
 ...
 */
static LonStatusCode CheckUplinkCompleted(
#if LINK_IS(MULTIPLE_USB_MIPS)
        int iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        bool *completed)
    ...
    if (state->uplink_msg_index == 1) {  // Length is at index 0; cmd is at index 1
        state->uplink_msg_length =
                state->uplink_buffer.usb_ni_data_frame
                        .short_pdu_length;  // TODO: Verify this is correct and that
        // short_pdu_length is populated at this point in
        // the code; if so, change the condition to check
        // for short_pdu_length instead of index 1 and
        // use the field instead of index 0
    } else if (state->uplink_msg_length == EXT_LENGTH && state->uplink_msg_index == 3) {
        // Get the extended length; ((uint8_t
        // *)&state->uplink_buffer.usb_ni_data_frame)[0] is still EXT_LENGTH
        state->uplink_msg_length =
        ...
        uint8_t ni_cmd =
                state->uplink_buffer.usb_ni_data_frame
                        .ni_command;  // TODO: Verify this is correct and that ni_command is
        // populated at this point in the code; if so, use the
        // field instead of index 1
        if ((ni_cmd & NI_CMD_MASK) == LonNiDriverCmdMask || ni_cmd == LonNiLayerModeCmd) {
            ...
            OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
                    "CheckUplinkCompleted: Received 0x%02X network interface "
                    "command with %d bytes",
                    ni_cmd, state->uplink_msg_length);
            OsalPrintMessage(DETAIL_TRACE_LOG, "CheckUplinkCompleted: ",
                    (uint8_t *)&state->uplink_buffer.usb_ni_data_frame,
                    state->uplink_msg_length + 2);
            switch (ni_cmd) {
            case LonNiResetDeviceCmd:
                ...
                if (state->uplink_buffer.usb_ni_data_frame.short_pdu_length) {
                    ...
                    state->lon_stats.channel_type_id =
                            state->uplink_buffer.usb_ni_data_frame.pdu[1];
                    ...
                    if (state->lon_stats.l2_l5_mode) {
                        strncpy(lon_ni_layer_mode_name, "layer 2",
                                sizeof(lon_ni_layer_mode_name));
                    }
                    OsalPrintLog(INFO_LOG, LonStatusNoError,
                    ...
                    OsalPrintLog(INFO_LOG, LonStatusNoError,
                            "CheckUplinkCompleted: NI Reset received, no data");
                }
                state->uplink_buffer.usb_ni_data_frame.short_pdu_length = 0;
                state->uplink_msg_length = 1;  // Adjust length for no data (TODO: Check
                                               // this and correct checksum)
                if (state->wait_for_reset) {
                    state->wait_for_reset = false;
                    state->have_reset =
                            true;  // Signal reset received to ProcessDownlinkRequests()
                }
                // TODO: Clear any pending transactions?
                // TODO: Alternate handling if not waiting for reset, for example if
                // reset is unexpected or unsolicited
                break;
            case LonNiCrcError:
                OsalPrintLog(INFO_LOG, LonStatusNoError,
                        "CheckUplinkCompleted: NI CRC error received");
                Increment32(state->lon_stats.uplink.rx_crc_errors);
                break;
            case LonNiIncomingCmd:
                OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
                        "CheckUplinkCompleted: Received unrecognized %d "
                        "byte NI "
                        "incoming command",
                        state->uplink_buffer.usb_ni_data_frame.short_pdu_length);
                break;
            case LonNiIncomingL2Cmd:
                if (state->uplink_buffer.usb_ni_data_frame.short_pdu_length ==
                                UNIQUE_ID_MSG_CODE_OFFSET + 4 &&
                        state->uplink_buffer.usb_ni_data_frame
                                        .pdu[UNIQUE_ID_MSG_CODE_OFFSET] == IzotNmWink) {
                    // Process uplink network Wink message
                    OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
                            "CheckUplinkCompleted: Received uplink Wink "
                            "message");
#if LINK_IS(USB_MIP)
                    IdentifyLonNi();
#else   // LINK_IS(MULTIPLE_USB_MIPS)
...
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
                } else if (state->uplink_buffer.usb_ni_data_frame.short_pdu_length ==
                                   BROADCAST_MSG_CODE_OFFSET + 18 &&
                           state->uplink_buffer.usb_ni_data_frame
                                           .pdu[BROADCAST_MSG_CODE_OFFSET] ==
                                   IzotNmServicePin) {
                    // Log received unique ID message
                    OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
                            "CheckUplinkCompleted: Received uplink Service Pin "
                            "message from %2.2X%2.2X:%2.2X%2.2X:%2.2X%2.2X",
                            state->uplink_buffer.usb_ni_data_frame
                                    .pdu[BROADCAST_MSG_CODE_OFFSET + 1],
                            state->uplink_buffer.usb_ni_data_frame
                                    .pdu[BROADCAST_MSG_CODE_OFFSET + 2],
                            state->uplink_buffer.usb_ni_data_frame
                                    .pdu[BROADCAST_MSG_CODE_OFFSET + 3],
                            state->uplink_buffer.usb_ni_data_frame
                                    .pdu[BROADCAST_MSG_CODE_OFFSET + 4],
                            state->uplink_buffer.usb_ni_data_frame
                                    .pdu[BROADCAST_MSG_CODE_OFFSET + 5],
                            state->uplink_buffer.usb_ni_data_frame
                                    .pdu[BROADCAST_MSG_CODE_OFFSET + 6]);
                } else {
                    OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
                            "CheckUplinkCompleted: Received unrecognized "
                            "%d byte NI "
                            "incoming L2 command",
                            state->uplink_buffer.usb_ni_data_frame.short_pdu_length);
                }
            ...
            case LonNiFlushCompleteCmd:
                OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
                        "CheckUplinkCompleted: NI Flush Complete message "
                        "received");
                break;
            default:
            ...
            if (((unsigned int)state->uplink_msg_length + 2) >
                    (sizeof(LonDataFrame) + 2)) {
                OsalPrintLog(INFO_LOG, LonStatusNoError,
                        "CheckUplinkCompleted: Uplink message had invalid "
                        "length: %d",
                        state->uplink_msg_length);
            } else {
                // Queue it
 ...
 */
// TODO: Fill in the additional fields for uplink_buffer
#if LINK_IS(USB_MIP)
static LonStatusCode ProcessUplinkMessage(void)
{
    LonUsbLinkState *state = &iface_state;
...
static LonStatusCode ProcessUplinkMessage(int iface_index)
{
    ...
    if (state->wait_for_uid) {
        ...
        if (copy_length >= 23 &&
                ...
                state->uplink_buffer.usb_ni_data_frame.pdu[14] ==
                        (IzotNmReadMemory &
                                (IZOT_NM_OPCODE_MASK | IZOT_NM_RESPONSE_SUCCESS))) {
            int i;
            for (i = 0; i < 6; i++) {
                state->uid[i] =
                        state->uplink_buffer.usb_ni_data_frame
                                .pdu[i + 15];  // TODO: Verify the correct offset for
                                               // the UID bytes in the message
            }
            state->wait_for_uid = false;
            state->have_uid = true;
            OsalPrintLog(INFO_LOG, LonStatusNoError,
                    "ProcessUplinkMessage:Unique ID "
                    "%2.2X%2.2X:%2.2X%2.2X:%2.2X%2.2X",
                    state->uid[0], state->uid[1], state->uid[2], state->uid[3],
                    state->uid[4], state->uid[5]);
        }
        ...
        // TODO: Replace buf with direct access to uplink_buffer fields and verify
        // the correct offsets for the response code and length fields in the
        // message
        if (copy_length >= 6 &&
                state->uplink_buffer.usb_ni_data_frame.ni_command == 0xE0 &&
                state->uplink_buffer.usb_ni_data_frame.short_pdu_length == 5) {
            IzotLonInterfaceStatusResponse *response =
                    (IzotLonInterfaceStatusResponse *)&state->uplink_buffer
                            .usb_ni_data_frame.pdu[0];
            state->wait_for_status = false;
            ...
            OsalPrintLog(INFO_LOG, LonStatusNoError,
                    "ProcessUplinkMessage:LON interface model %d, firmware "
                    "version %d.%d, layer %d mode, status %d",
                    response->model_id, response->fw_version / 10,
                    response->fw_version % 10, response->layer_mode ? 2 : 5,
                    response->status);
        }
    } else {
...
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
        OsalPrintLog(DETAIL_TRACE_LOG, status,
                "ProcessUplinkMessage: Queued uplink message with %d bytes", copy_length);
        OsalPrintMessage(DETAIL_TRACE_LOG, "ProcessUplinkMessage: ",
                (const uint8_t *)&state->uplink_buffer.usb_ni_data_frame, copy_length);
    }
    return status;
}
...
/*
 * Closes a LON USB network interface (multiple USB MIPS only).
 * Parameters:
 *   iface_index: interface index returned by OpenLonUsbLink()
 * Returns:
 *   LonStatusNoError on success; LonStatusCode error code if unsuccessful
...
static LonStatusCode InitIfaceStates(void)
{
    ...
    LonUsbLinkState *state = &iface_state;
#else   // LINK_IS(MULTIPLE_USB_MIPS)
    for (int iface_index = 0; iface_index < MAX_IFACE_STATES && LON_SUCCESS(status);
            iface_index++) {
    ...
    state->wait_for_reset = false;  // Set to true in OpenLonUsbLink() while
                                    // waiting for an uplink reset command
    state->have_reset = false;      // Set to true when a reset command is received in
                                    // CheckUplinkCompleted()
    state->wait_for_null = false;   // Set to true in OpenLonUsbLink() while waiting
                                    // for an uplink null frame
    state->have_null = false;       // Set to true when a null frame is received in
                                    // CheckUplinkCompleted()
    state->wait_for_uid = false;    // Set to true in RequestUsbNiUid()
    state->have_uid =
            false;  // Set to true when UID is received in ProcessUplinkMessage()
    state->wait_for_status = false;  // Set to true in OpenLonUsbLink() while
                                     // waiting for an uplink status response
    state->have_status = false;      // Set to true when a status response is received
                                     // in CheckUplinkCompleted()
    state->ready = false;            // Set to true when interface is ready for use
    ...
    state->configured_iface_mode = LON_IFACE_MODE_LAYER2;  // Default to layer 2 mode
    state->current_iface_mode =
            LON_IFACE_MODE_UNKNOWN;  // Default to unknown mode for startup
    ...
    state->uplink_state = UPLINK_IDLE;
    state->uplink_ack_timer = 0;
    state->uplink_ack_timeout =
            STARTUP_ACK_WAIT_TIME;  // Use longer timeout to allow LON interface reset
    state->uplink_ack_timeouts = 0;
    state->uplink_ack_timeout_phase = 0;
    state->uplink_duplicate = false;
    state->uplink_frame_error = false;
    ...
    if (((status = QueueInit(&state->lon_usb_downlink_normal_queue, "link layer downlink",
                  sizeof(LonUsbQueueBuffer), MAX_LON_DOWNLINK_BUFFERS)) !=
                LonStatusNoError) ||
            ((status = QueueInit(&state->lon_usb_downlink_priority_queue,
                      "link layer priority downlink", sizeof(LonUsbQueueBuffer),
                      MAX_LON_DOWNLINK_BUFFERS)) != LonStatusNoError)) {
        OsalPrintLog(ERROR_LOG, LonStatusStackInitializationFailure,
                "InitIfaceStates: Failed to initialize link layer "
                "downlink queue");
        return status;
    }
    // Allocate and initialize LON USB uplink queues
    if (((status = QueueInit(&state->lon_usb_uplink_normal_queue, "link layer uplink",
                  sizeof(LonUsbQueueBuffer), MAX_LON_UPLINK_BUFFERS)) !=
                LonStatusNoError) ||
            ((status = QueueInit(&state->lon_usb_uplink_priority_queue,
                      "link layer priority uplink", sizeof(LonUsbQueueBuffer),
                      MAX_LON_UPLINK_BUFFERS)) != LonStatusNoError)) {
        OsalPrintLog(ERROR_LOG, LonStatusStackInitializationFailure,
                "InitIfaceStates: Failed to initialize link layer uplink "
                "queue");
        return status;
    }
    // Initialize the LON USB uplink ring buffer (capacity set to full internal
    // max)
    if (!LON_SUCCESS(status = RingBufferInit(&state->lon_usb_uplink_ring_buffer,
                             RING_BUFFER_MAX_CAPACITY))) {
        OsalPrintLog(ERROR_LOG, status,
                "InitIfaceStates: Failed to initialize LON USB uplink "
                "ring buffer");
        return status;
    }
    RingBufferClear(&state->lon_usb_uplink_ring_buffer);
    state->lon_stats.usb_rx.capacity = RING_BUFFER_MAX_CAPACITY;
    state->lon_stats.usb_rx.max_occupancy =
            RingBufferSize(&state->lon_usb_uplink_ring_buffer);
...
#endif
OsalPrintLog(INFO_LOG, status,
        "Initialized %d LON interface states with %d byte uplink ring "
        "buffer and %d entry uplink and downlink queues",
        MAX_IFACE_STATES, RING_BUFFER_MAX_CAPACITY, MAX_LON_UPLINK_BUFFERS);
return status;
}

/*
 * Assigns a free interface state (multiple USB MIPS only).
 * Parameters:
 *   None
 * Returns:
...
/*
 * Gets a pointer to an interface state by index (multiple USB MIPS only).
 * Parameters:
...
static LonUsbLinkState *GetIfaceState(int index)
{
    ...
    if (!iface_state[index].assigned) {
        OsalPrintLog(INFO_LOG, LonStatusNoError, "GetIfaceState: index %d not assigned",
                index);
...
/*
 * Counts the number of free (unassigned) slots (multiple USB MIPS only).
 * Parameters:
 *   None
 * Returns:
 ...
 * ----------------------------------------------------------
 * Head:   LonUsbLinkState.downlink_queue points to first node, or NULL if
 * empty. Nodes:  Each node embeds a LonUsbQueueBuffer in entry->data. Ops:
 *  - QueueDownlinkBuffer: copy src into a new node; if head NULL set head, else
 * append.
 ...
 *   LonStatusNoError on success; LonStatusCode error code if unsuccessful
 */
static LonStatusCode ReadDownlinkBuffer(
#if LINK_IS(MULTIPLE_USB_MIPS)
        int iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        LonUsbQueueBuffer *dst)
{
#if LINK_IS(USB_MIP)
    LonUsbLinkState *state = &iface_state;
#else   // LINK_IS(MULTIPLE_USB_MIPS)
    ...
    if (!buffer) {
        ...
        OsalPrintLog(DETAIL_TRACE_LOG, status,
                "ReadDownlinkBuffer: Read %zu byte entry from %s queue, new "
                "entry count %d, head index %d (%p), tail index %d (%p)",
                dst->buf_size, source_queue->queueName, source_queue->queueEntries,
                source_queue->headIndex, source_queue->head, source_queue->tailIndex,
                source_queue->tail);
        OsalPrintMessage(DETAIL_TRACE_LOG, "ReadDownlinkBuffer: ", (const uint8_t *)dst,
                dst->buf_size);
    }
 ...
 *   LonStatusNoError on success; LonStatusCode error code if unsuccessful
 */
static LonStatusCode FlushDownlinkQueue(
#if LINK_IS(MULTIPLE_USB_MIPS)
        int iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        MessagePriorityLevel priority)
{
#if LINK_IS(USB_MIP)
    LonUsbLinkState *state = &iface_state;
#else   // LINK_IS(MULTIPLE_USB_MIPS)
 ...
 *   LonStatusNoError on success; LonStatusCode error code if unsuccessful
 */
static LonStatusCode QueueDownlinkBuffer(
#if LINK_IS(MULTIPLE_USB_MIPS)
        int iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        LonUsbQueueBuffer *src)
{
#if LINK_IS(USB_MIP)
    LonUsbLinkState *state = &iface_state;
#else   // LINK_IS(MULTIPLE_USB_MIPS)
    ...
    if (!src) {
        OsalPrintLog(ERROR_LOG, LonStatusInvalidParameter,
                "QueueDownlinkBuffer: Invalid buffer");
        return LonStatusInvalidParameter;
    }
    Queue *queue;
    ...
    if (!tail) {
        ...
        QueueWrite(queue);
        OsalPrintLog(DETAIL_TRACE_LOG, LonStatusNoError,
                "QueueDownlinkBuffer: Wrote %zu byte entry to %s queue, new "
                "entry count %d, head index %d (%p), tail index %d (%p)",
                src->buf_size, queue->queueName, queue->queueEntries, queue->headIndex,
                queue->head, queue->tailIndex, queue->tail);
        OsalPrintMessage(DETAIL_TRACE_LOG, "QueueDownlinkBuffer: ", (const uint8_t *)tail,
                src->buf_size);
    }
    OsalUnlockMutex(&state->queue_lock);
    return status;
...
static size_t GetDownlinkBufferCount(void)
{
    ...
    if (cnt != last_downlink_buffer_count) {
        ...
        OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
                "GetDownlinkBufferCount: Downlink buffer count: %zu", cnt);
    }
    return cnt;
}
 ...
 *   iface_index: interface index returned by OpenLonUsbLink()
 *             (multiple USB MIPS only)
 *   out_msg: pointer to LonDataFrame
 * Returns:
 ...
 * Ops:
 *  - QueueUplinkBuffer: copy src into a new node; if head NULL set head, else
 * append.
 ...
 *   LonStatusNoError on success; LonStatusCode error code if unsuccessful
 */
static LonStatusCode ReadUplinkBuffer(
#if LINK_IS(MULTIPLE_USB_MIPS)
        int iface_index,
#endif  // LINK_IS(MULTIPLE_USB_MIPS)
        LonUsbQueueBuffer *dst)
...
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
    LonUsbFrameCommand frame_cmd = IZOT_GET_ATTRIBUTE(state->uplink_buffer.frame_header,
            IZOT_U50_FRAME_CODE_CMD);
    unsigned int sequence_num = IZOT_GET_ATTRIBUTE(state->uplink_buffer.frame_header,
            IZOT_U50_FRAME_CODE_SEQ);
    bool ack_flag = IZOT_GET_ATTRIBUTE(state->uplink_buffer.frame_header,
                            IZOT_U50_FRAME_CODE_ACK) != 0;
    uint8_t parameter = state->uplink_buffer.frame_header.parameter;
    uint8_t checksum = state->uplink_buffer.frame_header.checksum;
    OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
            "%sReceived uplink code packet with 0x%02X frame command, %u "
            "sequence, %s ACK flag, 0x%02X parameter",
            prefix, frame_cmd, sequence_num, (ack_flag) ? "Set" : "Clear", parameter);
    OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
            "%sUplink state %s, downlink state %s, checksum 0x%02X", prefix,
            uplink_state_names[state->uplink_state],
            downlink_state_names[state->downlink_state], checksum);
}
...
static void IdentifyLonNi(void)
{
    ...
    {
        OsalPrintLog(INFO_LOG, LonStatusNoError,
                "IdentifyLonNi: LON Wink command received");
    }
#endif  // LINK_IS(USB_MIP) || LINK_IS(MULTIPLE_USB_MIPS)
