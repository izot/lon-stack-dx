...
#include "lon_udp/ipv4_to_lon_udp.h"  // IWYU pragma: keep
...
#define LS_MAP_STATE_DERIVED                                                             \
    1  // The IP address should be derived from                                     \
            // the LS address
#define LS_MAP_STATE_ARBITRARY                                                           \
    2  // The IP address is in the                                                  \
 ...
 */
static LsMappingInfo *findMappingInfo(const IzotByte *pDomainId, IzotByte domainLen,
 ...
 */
static LsMappingInfo *findAvailMappingInfo(const IzotByte *pDomainId, IzotByte domainLen,
        IzotByte subnetId, IzotByte nodeId)
{
    LsMappingInfo *pMapInfo = NULL;
 ...
 */
IzotByte Ipv4GetArbitrarySourceAddress(void *lsMappingHandle, IzotByte *pSourceIpAddress,
        const IzotByte *pDomainId, int domainIdLen, IzotByte *pEnclosedSource)
{
...
#endif  // PROTOCOL_IS(LON_IPV6)
    {
        ...
        foundAddress = Ipv4IsUnicastAddressSupported(lsMappingHandle, pSourceIpAddress);
        ...
        if (!foundAddress) {
            ...
            includeDomain = TRUE;
            GenerateLonUdpAddress(pDomainId, domainIdLen, 1, 1, domainIpPrefix);
            const IzotByte *pSourceAddr = ownIpAddress;
            if (pArbitraryAddress == NULL) {
                pArbitraryAddress = pSourceAddr;  // First available
            }
            if (memcmp(pSourceAddr, domainIpPrefix, IPV4_LON_UDP_IPADDR_DOMAIN_LEN) ==
                    0) {
    ...
    if (includeDomain) {
        ...
        memcpy(&pEnclosedSource[IPV4_LSUDP_NPDU_OFF_ARB_SOURCE_DM], pDomainId,
                domainIdLen);
    }
    ...
    if (pArbitraryAddress != NULL) {
        ...
        pEnclosedSource[IPV4_LSUDP_NPDU_OFF_ARB_SOURCE_SUBNET] =
                pSourceIpAddress[IPV4_LON_UDP_UCADDR_OFF_SUBNET];
        pEnclosedSource[IPV4_LSUDP_NPDU_OFF_ARB_SOURCE_NODE] =
                pSourceIpAddress[IPV4_LON_UDP_UCADDR_OFF_NODE] & NODE_ID_MASK;
 ...
 */
IzotByte Ipv4GetArbitraryDestAddress(void *lsMappingHandle, const IzotByte *pDomainId,
        IzotByte domainLen, IzotByte subnetId, IzotByte nodeId, IzotByte ipv1AddrFmt,
        IzotByte *pDestIpAddress, IzotByte *pEnclosedDest)
{
 ...
 * void
 */
void Ipv4SetArbitraryAddressMapping(void *lsMappingHandle,
        const IzotByte *pArbitraryIpAddr, const IzotByte *pDomainId, IzotByte domainLen,
        IzotByte subnetId, IzotByte nodeId)
{
    LsMappingInfo *pMapInfo = findMappingInfo(pDomainId, domainLen, subnetId, nodeId);
 ...
 * void
 */
void Ipv4SetDerivedAddressMapping(void *lsMappingHandle, const IzotByte *pDomainId,
        IzotByte domainLen, IzotByte subnetId, IzotByte nodeId)
{
    LsMappingInfo *pMapInfo = findMappingInfo(pDomainId, domainLen, subnetId, nodeId);
 ...
 * void
 */
void Ipv4SetDerivedSubnetsMapping(void *lsMappingHandle, const IzotByte *pDomainId,
        IzotByte domainLen, IzotByte set, const IzotByte *pSubnets)
{
 ...
 */
IzotByte Ipv4IsUnicastAddressSupported(void *lsMappingHandle, IzotByte *ipAddress)
{
#if LINK_IS(UDP)
 ...
 */
void UpdateMapping(IzotByte *pDomainId, IzotByte domainLen, IzotByte subnetId,
        IzotByte nodeId, const IzotByte *addr)
{
    OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
            ...
            (domainLen == 3 ? pDomainId[1] : 0), addr[0], addr[1], addr[2], addr[3]);
    ...
    if (addr[IPV4_LON_UDP_UCADDR_OFF_SUBNET] == subnetId &&
            addr[IPV4_LON_UDP_UCADDR_OFF_NODE] == nodeId) {
        if ((domainLen == 0 && addr[0] == 0xc0 && addr[1] == 0xa8) ||
                (domainLen == 1 && addr[0] == 0x0A) ||
                (domainLen == 3 && addr[0] == pDomainId[0] && addr[1] == pDomainId[1])) {
            OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
                    "mapping updated to the ls derived\r\n");
            pMapInfo->state = LS_MAP_STATE_DERIVED;
        } else {
            pMapInfo->state = LS_MAP_STATE_ARBITRARY;
            OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
                    "mapping updated to the arbitrary\r\n");
        }
    } else {
        pMapInfo->state = LS_MAP_STATE_ARBITRARY;
        OsalPrintLog(PACKET_TRACE_LOG, LonStatusNoError,
